...
`20260920093203_native_routine_editing.sql` adds actor/household/operation receipts and strict patch/version validation. It delegates actual mutation to the audited legacy CAS wrapper. Its internal legacy key is a fresh server-generated UUID, preventing clients of the older household-scoped RPC from pre-seeding native retry keys. New retries use the native receipt before invoking the legacy engine, so they never reapply after a later edit.

Only supplied fields are translated. Validation placeholders never reach storage. New recurrence rules are checked against supported date arithmetic even for paused routines, and resulting open dates are range-checked transactionally. Existing unrelated fields remain untouched. Three initial native transaction cases pass; this is not yet API/device verification or a complete native edit slice.

...
Storage/contracts are a reviewable foundation, not a completed handover flow. The API candidate supplies effective-owner mapping and authenticated pending/request/respond endpoints with strict receipt binding. The native candidate adds request/inbox controls, exact-attempt recovery and an account-scoped readonly SQLite cache. Corresponding private AI actions and device acceptance must land before this workflow is counted complete. Item notifications and physical device acceptance remain separate M8/M4 work. Legacy notification semantics are unchanged. No production migration has been applied; legacy writer cutover and a full authorized migration rehearsal remain M9 gates.

## Consistent native refresh

The native chore/handovers cache is populated by one authenticated `nest_chore_snapshot` RPC. Both public and private wrappers are `STABLE` security invokers with an empty search path; membership, bounded roster, chores and pending requests share the calling query's MVCC snapshot. Separate volatile helpers/HTTP reads are deliberately not composed into a fresh cache. Strict shared validation also rejects pending requests whose current owner, date or title does not match the chore. A controlled concurrent-acceptance database test and the real native HTTP journeys cover this boundary. Snapshot reads can become stale after returning, so mutations still validate the exact live target and consent baseline.

PostgreSQL's [function volatility documentation](https://www.postgresql.org/docs/current/xfunc-volatility.html) specifies the calling-query snapshot used by `STABLE` functions. The additive migration remains gated; no production execution occurred.

...
`nest_read_legacy_recurring` returns up to 20 household-authorized rules in ID order with stable continuation. Every row is explicitly `legacy_draft_only`. It preserves supported edit versions and dates in tagged values, converts valid safe integer splits to decimal strings without rounding, and returns pending/posted/dismissed counts and latest retained draft date. The legacy DDL also permits infinite/out-of-range dates or timestamps and invalid allocation arrays. Such rows remain visible with explicit unsupported date/version or needs-review split states; original stored values remain untouched. A separate unsupported-draft-date count detects those dates even when a supported date is the maximum. None of these states supplies valid adoption terms. Descriptions preserve the legacy ASCII-space-trim constraint rather than new-expense validation; whitespace-only Unicode labels or long padding remain raw retained data, not validated new expense terms. It separately counts posted drafts lacking an event and unposted drafts with an event. Nonzero discrepancy counts require reconciliation; the read does not correct them.
...
The protected native legacy list and paginated draft review are implemented. Draft review preserves original nullable terms and the exact financial-event link, including status/link discrepancies; it never substitutes the current rule configuration. Native confirm/dismiss/reconciliation decisions remain to implement. Explicit adoption must retain legacy-to-native identity, require the exact reviewed legacy version and selected future terms, serialize with old rule edits/generation, and prevent duplicate posting for retained draft periods. Pending legacy obligations need a visible reconciliation/decision path. Neither old active status nor the AI reading this inventory grants consent. Native rules must not be created as unlinked copies to bypass this reconciliation.

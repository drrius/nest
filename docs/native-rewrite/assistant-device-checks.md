...
- Read chores/groceries and follow their links. Request a chore completion and grocery add/edit/check/uncheck/remove, then compare the saved receipt and actual native screen. Drop the connection during a write and verify reload recovers one committed result without repeating it. Ask for multiple writes with a deliberately stale first version; later queued writes must stop. No financial approval, meal proposal or memory UI is claimed by this slice.

...
The next manual replacement slice keeps the original entry as soft-removed history and creates a new independently identified entry in the same explicitly supplied date/slot. A search of the pinned legacy source found no dedicated replacement transaction to reuse. Do not implement this as two client requests: a failed second request would otherwise leave an empty slot and could lose the original meal's preparation state.

Compose the audited private removal and placement commands inside one new authorized transaction with one immutable outer receipt. Require the original entry ID, exact week revision and its expected date/slot before any mutation. Acquire the week counter before the entry, authorize current membership before receipt replay, and use fresh server-generated internal operation IDs for the two child commands. Their receipts and actual linked preparation closure must roll back if placement or the final outer receipt fails. The old entry's recipe, notes, provenance and grocery history are retained. Active dependent leftovers block replacement through the shared removal invariant; no automatic dependent removal. A new one-off entry does not inherit the old recipe or preparation, and ingredients are never automatically added.

Each actual entry change advances the existing shared week counter, so replacement advances it twice; neither suppress the legacy-compatible revision trigger nor pretend this is one increment. The response must bind the old and new entry IDs, date/slot, exact resulting revision and any skipped preparation ID. Test concurrent replacement, partner edits, stale/occupied/foreign input, revoked replay, forced failure after removal, receipt failure after placement, and immutable recovery after later changes. Follow with native replacement confirmation and the corresponding private AI action. Saved-meal selection remains a separate audited payload mode to implement with full recipe snapshots; a title-only replacement does not complete that requirement.

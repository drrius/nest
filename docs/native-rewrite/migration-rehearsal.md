...
Recurring creation/update, pause/cancel, resume, variable-cycle and manual-link approval readers are also retained. Twenty-one independent proposals exercise consumed, denied and pending states, exact owner recovery, rejection of partner/outsider/anonymous reads, and refused decisions/execution. Approval rows and all recurring rules/revisions/cycles remain unchanged, followed by final financial reconciliation. Manual linkage retains its existing source expense without posting another. Legacy approval variants and native execution remain outside this fixture proof.

The secondary settlement/adjustment/recurring household has independent synthetic members and valid new-command amounts. The original retained-history stress household, including its deliberately oversized balance, is preserved unchanged. No production household is read or modified. The latest combined local report is `/tmp/nest-recurring-variants-recovery.json`; reports in `/tmp` are local evidence, not durable release artifacts. Exact review/CI/merge state is recorded separately in `docs/progress.md`.

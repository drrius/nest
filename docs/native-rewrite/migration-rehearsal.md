...
The secondary settlement/adjustment/recurring household has independent synthetic members and valid new-command amounts. The original retained-history stress household, including its deliberately oversized balance, is preserved unchanged. No production household is read or modified. The latest combined local report is `/tmp/nest-adjustment-approval-recovery.json`; reports in `/tmp` are local evidence, not durable release artifacts. Exact review/CI/merge state is recorded separately in `docs/progress.md`.

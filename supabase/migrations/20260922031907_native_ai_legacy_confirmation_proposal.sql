-- GATED: AI proposes complete reviewed expense terms; only native approval posts them.
create function private.nest_canonical_legacy_confirmation_proposal(p_input jsonb)
...
begin
  if p_input is null or jsonb_typeof(p_input) is distinct from 'object'
    ...
    raise exception 'Invalid legacy confirmation proposal' using errcode='22023'; end if;
  v_header:=private.nest_canonical_legacy_dismissal_proposal(p_input-'expense');
  return v_header||jsonb_build_object('expense',private.nest_canonical_expense(p_input->'expense'));
end;
$$;
revoke all on function private.nest_canonical_legacy_confirmation_proposal(jsonb) from public,anon,authenticated,service_role;
create function private.nest_propose_legacy_confirmation(p_household uuid,p_operation uuid,p_input jsonb)
returns jsonb language plpgsql security definer set search_path='' as $$
declare v_actor uuid:=auth.uid(); v_draft public.expense_drafts; v_review jsonb;
  v_payload jsonb; v_approval uuid; v_members integer;
begin
  ...
  p_input:=private.nest_canonical_legacy_confirmation_proposal(p_input);
  ...
  if not found or v_draft.recurring_expense_rule_id is null then raise exception 'Draft unavailable' using errcode='42501'; end if;
  ...
  perform private.nest_expense_payload(p_input->'expense',p_household);
  v_payload:=jsonb_build_object('draftId',v_draft.id,'ruleId',v_draft.recurring_expense_rule_id,
    'reviewToken',v_review->>'reviewToken','expense',p_input->'expense');
  v_approval:=private.nest_propose_action(p_household,p_operation,'recurring.confirm-legacy-draft',1,v_payload);
  return private.nest_read_legacy_confirmation_approval(p_household,v_approval);
end;
$$;
revoke all on function private.nest_propose_legacy_confirmation(uuid,uuid,jsonb) from public,anon,authenticated,service_role;

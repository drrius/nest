-- GATED grocery source on the existing immutable delivery journal. No hosted sends.
alter table private.nest_push_deliveries
  add column grocery_id uuid references private.nest_grocery_reminder_outbox(id),
  drop constraint nest_push_exactly_one_source,
  add constraint nest_push_exactly_one_source check(num_nonnulls(outbox_id,summary_id,chore_id,meal_id,grocery_id)=1),
  add constraint nest_push_grocery_installation unique(grocery_id,installation_id);

create function private.nest_grocery_push_current(p_outbox private.nest_grocery_reminder_outbox,p_device private.nest_push_devices)
returns boolean language sql volatile security definer set search_path='' as $$
  select p_outbox.state='pending' and p_outbox.due_at<=clock_timestamp()
    ...
    and private.nest_grocery_reminder_current(p_outbox)
...
$$;
revoke all on function private.nest_grocery_push_current(private.nest_grocery_reminder_outbox,private.nest_push_devices) from public,anon,authenticated,service_role;

create function private.nest_prepare_grocery_push(p_outbox uuid,p_installation uuid)
returns uuid language plpgsql security definer set search_path='' as $$
declare v_outbox private.nest_grocery_reminder_outbox; v_device private.nest_push_devices; v_id uuid;
begin
  perform pg_advisory_xact_lock(hashtextextended('nest:push-device-enrollment',0));
  select * into v_outbox from private.nest_grocery_reminder_outbox where id=p_outbox for share;
  select * into v_device from private.nest_push_devices where installation_id=p_installation for share;
  if not coalesce(private.nest_grocery_push_current(v_outbox,v_device),false) then return null; end if;
  insert into private.nest_push_deliveries(grocery_id,installation_id,registration_revision)
    values(p_outbox,p_installation,v_device.revision)
    on conflict(grocery_id,installation_id) do update set registration_revision=excluded.registration_revision,state='ready'
      where private.nest_push_deliveries.state in ('ready','cancelled') and private.nest_push_deliveries.attempt_id is null
    returning id into v_id;
  return v_id;
end;
$$;
revoke all on function private.nest_prepare_grocery_push(uuid,uuid) from public,anon,authenticated,service_role;

create function private.nest_begin_grocery_push_once(p_delivery uuid)
returns jsonb language plpgsql security definer set search_path='' as $$
declare v_delivery private.nest_push_deliveries; v_outbox private.nest_grocery_reminder_outbox;
  v_device private.nest_push_devices; v_item public.grocery_items; v_attempt uuid:=gen_random_uuid();
begin
  perform pg_advisory_xact_lock(hashtextextended('nest:push-device-enrollment',0));
  select * into v_delivery from private.nest_push_deliveries where id=p_delivery for update;
  if v_delivery.id is null or v_delivery.state<>'ready' or v_delivery.grocery_id is null then return null; end if;
  select * into strict v_outbox from private.nest_grocery_reminder_outbox where id=v_delivery.grocery_id for share;
  ...
  select * into v_item from public.grocery_items
    where household_id=v_outbox.household_id and id=v_outbox.item_id for share;
  perform 1 from public.nest_grocery_reminders where household_id=v_outbox.household_id and item_id=v_outbox.item_id for share;
  ...
  if v_device.revision is distinct from v_delivery.registration_revision
    or not coalesce(private.nest_grocery_push_current(v_outbox,v_device),false) then
  ...
  return jsonb_build_object('version',1,'deliveryId',p_delivery,'attemptId',v_attempt,
    ...
    'token',v_device.token,'householdId',v_outbox.household_id,'itemId',v_outbox.item_id);
exception when lock_not_available then return null;
end;
$$;
revoke all on function private.nest_begin_grocery_push_once(uuid) from public,anon,authenticated,service_role;
...
declare v_delivery private.nest_push_deliveries; v_outbox private.nest_renewal_reminder_outbox;
  v_grocery private.nest_grocery_reminder_outbox; v_meal private.nest_meal_reminder_outbox; v_chore private.nest_chore_reminder_outbox; v_summary private.nest_daily_summary_outbox; v_device private.nest_push_devices; v_reason text; v_recorded timestamptz; v_count integer;
begin
  perform pg_advisory_xact_lock(hashtextextended('nest:push-device-enrollment',0));
  select * into v_delivery from private.nest_push_deliveries where id=p_delivery for update;
  ...
  select * into v_device from private.nest_push_devices where installation_id=v_delivery.installation_id for share;
  if v_delivery.grocery_id is not null then

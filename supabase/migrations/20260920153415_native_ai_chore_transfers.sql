-- GATED additive candidate. Handover consent and private AI journal results commit atomically.
...
create function private.nest_validate_ai_chore_change(p_tool text,p_input jsonb)
...
$$;
revoke all on function private.nest_validate_ai_chore_change(text,jsonb) from public,anon,authenticated;

create function private.nest_validate_ai_routine_command(p_tool text,p_input jsonb)
returns void language plpgsql security invoker set search_path='' as $$
begin
...
$$;
revoke all on function private.nest_validate_ai_routine_command(text,jsonb) from public,anon,authenticated;

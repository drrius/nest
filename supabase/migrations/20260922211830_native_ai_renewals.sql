-- Reuse the native renewal command inside the existing private journal transaction.
create function private.nest_canonical_ai_renewal(p_tool text,p_input jsonb)
returns jsonb language plpgsql security invoker set search_path='' as $$
declare v_keys text[]; v_result jsonb;
begin
  if p_tool not in ('createRenewal','editRenewal','removeRenewal') or p_tool is null then
    raise exception 'Invalid renewal action' using errcode='22023'; end if;
  v_keys:=case p_tool when 'createRenewal' then array['fields']
    when 'editRenewal' then array['renewalId','expectedRevision','fields']
    else array['renewalId','expectedRevision'] end;
  if jsonb_typeof(p_input) is distinct from 'object' or pg_catalog.octet_length(p_input::text)>8192
    or not (p_input ?& v_keys) or p_input-v_keys<>'{}'::jsonb then
    raise exception 'Invalid renewal input' using errcode='22023'; end if;
  v_result:='{}'::jsonb;
  if p_tool<>'createRenewal' then
    v_result:=jsonb_build_object('renewalId',private.nest_expense_uuid(p_input->'renewalId',false),
      'expectedRevision',private.nest_expense_uuid(p_input->'expectedRevision',false));
  end if;
  if p_tool<>'removeRenewal' then
    v_result:=v_result||jsonb_build_object('fields',private.nest_renewal_fields(p_input->'fields'));
  end if;
  return v_result;
end;
$$;
revoke all on function private.nest_canonical_ai_renewal(text,jsonb) from public,anon,authenticated,service_role;

create function private.nest_dispatch_ai_renewal(p_household uuid,p_operation uuid,p_tool text,p_input jsonb)
returns jsonb language plpgsql security definer set search_path='' as $$
declare v_input jsonb:=private.nest_canonical_ai_renewal(p_tool,p_input);
begin
  if p_tool='createRenewal' then
    v_input:=v_input||jsonb_build_object('renewalId',p_operation,'expectedRevision',null);
  end if;
  return private.nest_change_renewal(p_household,p_operation,v_input,p_tool='removeRenewal');
end;
$$;
revoke all on function private.nest_dispatch_ai_renewal(uuid,uuid,text,jsonb) from public,anon,authenticated,service_role;

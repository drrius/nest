-- GATED: private settlement proposals. Models cannot approve or post money.
create function private.nest_canonical_settlement(p_input jsonb)
...
begin
  if p_input is null or jsonb_typeof(p_input) is distinct from 'object'
    ...
    or jsonb_typeof(p_input->'recipientId') is distinct from 'string' then
    raise exception 'Invalid settlement input' using errcode='22023';
  end if;
  v_payload:=p_input||jsonb_build_object('payerId',lower(p_input->>'payerId'),'recipientId',lower(p_input->>'recipientId'));
  perform private.nest_settlement_payload(v_payload);
  return v_payload;
end;
$$;
revoke all on function private.nest_canonical_settlement(jsonb) from public,anon,authenticated;

create function private.nest_propose_settlement(p_household uuid,p_operation uuid,p_input jsonb)
...
begin
  ...
  v_payload:=private.nest_canonical_settlement(p_input);
  perform private.lock_household_ledger(p_household);
  perform private.nest_validate_settlement_balance(p_household,v_payload);
  v_approval:=private.nest_propose_action(p_household,p_operation,'settlements.record',1,v_payload);
  return private.nest_read_settlement_approval(p_household,v_approval);
end;
$$;
revoke all on function private.nest_propose_settlement(uuid,uuid,jsonb) from public,anon,authenticated;

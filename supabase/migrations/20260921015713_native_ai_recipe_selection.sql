-- GATED additive candidate. Saved-recipe selection and private journal results commit atomically.
...
alter table public.nest_ai_commands add constraint nest_ai_commands_tool_name_check
  check(tool_name in ('placeRecipe','replaceWithRecipe','editRecipe','archiveRecipe','createRecipe','replaceMeal','moveMeal','removeMeal','placeMeal','completeChore','addGrocery','editGrocery','removeGrocery','checkGrocery',
...
$$;
-- Canonicalize only identity fields that are actually present; omitted patches stay omitted.
create or replace function private.nest_ai_recipe_edit_input(p_input jsonb)
returns jsonb language plpgsql immutable security invoker set search_path='' as $$
declare v_input jsonb; v_items jsonb:='[]'::jsonb; v_item jsonb;
begin
  v_input:=jsonb_set(p_input,'{definitionId}',to_jsonb(lower(p_input->>'definitionId')));
  if p_input->'ingredients'='null'::jsonb then return v_input; end if;
  for v_item in select value from jsonb_array_elements(p_input->'ingredients') loop
    if v_item->>'kind'='existing' then
      v_item:=jsonb_set(v_item,'{ingredientId}',to_jsonb(lower(v_item->>'ingredientId')));
      if v_item->'patch' ? 'categoryId' and v_item->'patch'->'categoryId'<>'null'::jsonb then
        v_item:=jsonb_set(v_item,'{patch,categoryId}',to_jsonb(lower(v_item->'patch'->>'categoryId')));
      end if;
    elsif v_item->'categoryId'<>'null'::jsonb then
      v_item:=jsonb_set(v_item,'{categoryId}',to_jsonb(lower(v_item->>'categoryId')));
    end if;
    v_items:=v_items||jsonb_build_array(v_item);
  end loop;
  return jsonb_set(v_input,'{ingredients}',v_items);
end;
$$;
create or replace function private.nest_dispatch_ai_recipe(p_household uuid,p_operation uuid,p_tool text,p_input jsonb)
returns jsonb language plpgsql security invoker set search_path='' as $$
begin
  if p_tool in ('placeRecipe','replaceWithRecipe') then
    p_input:=jsonb_set(p_input,'{definitionId}',to_jsonb(lower(p_input->>'definitionId')));
    if p_tool='replaceWithRecipe' then
      p_input:=jsonb_set(p_input,'{entryId}',to_jsonb(lower(p_input->>'entryId')));
    end if;
    return private.nest_select_recipe(p_household,p_operation,p_input,p_tool='replaceWithRecipe');
  end if;
  if p_tool='editRecipe' then
    return private.nest_edit_recipe(p_household,p_operation,private.nest_ai_recipe_edit_input(p_input));
  end if;
  if p_tool='archiveRecipe' then
    return private.nest_archive_recipe(p_household,p_operation,
      jsonb_set(p_input,'{definitionId}',to_jsonb(lower(p_input->>'definitionId'))));
  end if;
  if p_tool='createRecipe' then return private.nest_create_recipe(p_household,p_operation,p_input); end if;
  raise exception 'Invalid recipe tool' using errcode='22023';
end;
$$;
revoke all on function private.nest_ai_recipe_edit_input(jsonb),
  private.nest_dispatch_ai_recipe(uuid,uuid,text,jsonb) from public,anon,authenticated;

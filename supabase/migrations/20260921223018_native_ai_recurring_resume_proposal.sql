-- GATED: private prospective resume proposals; only native approval renews a mandate.
create function private.nest_canonical_recurring_resume_proposal(p_input jsonb)
returns jsonb language plpgsql immutable set search_path='' as $$
declare v_input jsonb;
begin
  if p_input is null or jsonb_typeof(p_input) is distinct from 'object' then
    raise exception 'Invalid recurring resume proposal' using errcode='22023'; end if;
  ...
  perform private.nest_recurring_resume_input(v_input);
  return v_input;
end;
$$;
revoke all on function private.nest_canonical_recurring_resume_proposal(jsonb) from public,anon,authenticated;
create function private.nest_propose_recurring_resume(p_household uuid,p_operation uuid,p_input jsonb)
returns jsonb language plpgsql security definer set search_path='' as $$
declare v_input jsonb; v_rule public.nest_recurring_rules; v_id uuid; v_members integer; v_approval uuid; v_execution private.nest_recurring_execution; v_from date; v_cycle jsonb;
begin
  ...
  -- Same operation-before-rule ordering as native decisions and the serialized review.
  perform pg_advisory_xact_lock(hashtextextended('nest:recurring-state-operation:'||auth.uid()::text||':'||p_household::text||':'||p_operation::text,0));
  ...
  v_input:=private.nest_canonical_recurring_resume_proposal(p_input);
  ...
  v_from:=(v_input->>'resumeFrom')::date;
  ...
  return private.nest_read_recurring_resume_approval(p_household,v_approval);
end;
$$;
revoke all on function private.nest_propose_recurring_resume(uuid,uuid,jsonb) from public,anon,authenticated;

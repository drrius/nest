-- GATED: private variable-cycle reviews and atomic native decisions. No scheduler permission.
-- READ COMMITTED operation fence ensures recovery observes earlier execution before retiring intent.
create function private.nest_read_variable_cycle_approval(p_household uuid,p_approval uuid)
...
begin
  ...
  select invocation_id into v_operation from public.nest_action_approvals
    where id=p_approval and household_id=p_household and actor_id=v_actor
      and command='recurring.record-cycle' and command_version=1;
  ...
  select jsonb_build_object('version',1,'actorId',a.actor_id,'householdId',a.household_id,'approval',
    ...
    where a.id=p_approval and a.household_id=p_household and a.actor_id=v_actor
      and a.command='recurring.record-cycle' and a.command_version=1;
  if not found then raise exception 'Not authorized' using errcode='42501'; end if;
  return v_result;
end;
$$;
revoke all on function private.nest_read_variable_cycle_approval(uuid,uuid) from public,anon,authenticated;
grant execute on function private.nest_read_variable_cycle_approval(uuid,uuid) to authenticated;
create function public.nest_read_variable_cycle_approval(p_household uuid,p_approval uuid)
returns jsonb language sql volatile security invoker set search_path='' as $$
  select private.nest_read_variable_cycle_approval($1,$2);
$$;
revoke all on function public.nest_read_variable_cycle_approval(uuid,uuid) from public,anon,authenticated;
grant execute on function public.nest_read_variable_cycle_approval(uuid,uuid) to authenticated;

create function private.nest_decide_variable_cycle(p_household uuid,p_operation uuid,p_payload jsonb,p_approval uuid,p_approved boolean)
...
begin
  ...
  perform private.nest_variable_cycle_input(p_payload);
  ...
  if p_approved and exists(select 1 from public.nest_recurring_cycle_receipts
    ...
    perform private.nest_record_variable_cycle(p_household,p_operation,p_payload,p_approval);
    return private.nest_read_variable_cycle_approval(p_household,p_approval);
  end if;
  ...
  v_command:='recurring.record-cycle';
  ...
  if not p_approved and v_row.status='denied' then return private.nest_read_variable_cycle_approval(p_household,p_approval); end if;
  perform private.nest_decide_action(p_approval,p_operation,v_command,1,p_payload,p_approved);
  if p_approved then perform private.nest_record_variable_cycle(p_household,p_operation,p_payload,p_approval); end if;
  return private.nest_read_variable_cycle_approval(p_household,p_approval);
end;
$$;
revoke all on function private.nest_decide_variable_cycle(uuid,uuid,jsonb,uuid,boolean) from public,anon,authenticated;
grant execute on function private.nest_decide_variable_cycle(uuid,uuid,jsonb,uuid,boolean) to authenticated;
create function public.nest_decide_variable_cycle(p_household uuid,p_operation uuid,p_payload jsonb,p_approval uuid,p_approved boolean)
returns jsonb language sql security invoker set search_path='' as $$
  select private.nest_decide_variable_cycle($1,$2,$3,$4,$5);
$$;
revoke all on function public.nest_decide_variable_cycle(uuid,uuid,jsonb,uuid,boolean) from public,anon,authenticated;
grant execute on function public.nest_decide_variable_cycle(uuid,uuid,jsonb,uuid,boolean) to authenticated;

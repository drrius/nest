-- GATED additive candidate. Requested notification choices use the native command; no OS permission or delivery.
...
alter table public.nest_ai_commands add constraint nest_ai_commands_tool_name_check
  check(tool_name in ('completeChore','addGrocery','editGrocery','removeGrocery','checkGrocery',
    'saveFoodPreferences','saveCookingPreferences','proposeMemory','removeMemory','saveNotificationPreferences'));

create function private.nest_validate_ai_notification_preferences(p_input jsonb)
...
begin
  if p_input is null or jsonb_typeof(p_input) is distinct from 'object'
    ...
    raise exception 'Invalid notification command' using errcode='22023';
  end if;
  if (p_input->>'expectedRevision')::numeric>9223372036854775807 then
    raise exception 'Invalid notification revision' using errcode='22023';
  end if;
  v_preferences:=p_input->'preferences';
  if jsonb_typeof(v_preferences) is distinct from 'object'
    or not (v_preferences ?& array['dailySummaryEnabled','dailySummaryTime','itemRemindersEnabled'])
    or v_preferences-array['dailySummaryEnabled','dailySummaryTime','itemRemindersEnabled']<>'{}'::jsonb
    or jsonb_typeof(v_preferences->'dailySummaryEnabled') is distinct from 'boolean'
    or jsonb_typeof(v_preferences->'dailySummaryTime') is distinct from 'string'
    or jsonb_typeof(v_preferences->'itemRemindersEnabled') is distinct from 'boolean' then
    raise exception 'Invalid notification preferences' using errcode='22023';
  end if;
  -- The shared native command validates the exact HH:mm wall-clock value.
end;
$$;
revoke all on function private.nest_validate_ai_notification_preferences(jsonb) from public,anon,authenticated;
...
begin
  ...
  select coalesce(jsonb_agg(part order by position),'[]'::jsonb) into v_parts
    ...
    where coalesce(part->>'type','') not in
      ('tool-completeChore','tool-addGrocery','tool-editGrocery','tool-removeGrocery','tool-checkGrocery','tool-saveFoodPreferences','tool-saveCookingPreferences','tool-proposeMemory','tool-removeMemory','tool-saveNotificationPreferences');

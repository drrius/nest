-- GATED additive candidate. Routine editing and private AI journaling commit atomically.
...
begin
  ...
  select coalesce(jsonb_agg(part order by position),'[]'::jsonb) into v_parts
    ...
    where coalesce(part->>'type','') not in
      ('tool-completeChore','tool-addGrocery','tool-editGrocery','tool-removeGrocery','tool-checkGrocery','tool-saveFoodPreferences','tool-saveCookingPreferences','tool-proposeMemory','tool-removeMemory','tool-saveNotificationPreferences','tool-createRoutine','tool-editRoutine');

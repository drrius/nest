-- GATED: private recurring resumption approval snapshots and atomic direct native decisions.
create function private.nest_read_recurring_resume_approval(p_household uuid,p_approval uuid)
...
create function public.nest_read_recurring_resume_approval(p_household uuid,p_approval uuid)
...
create function private.nest_decide_recurring_resume(p_household uuid,p_operation uuid,p_payload jsonb,p_approval uuid,p_approved boolean)
...
begin
  ...
  perform private.nest_recurring_resume_input(p_payload);
  ...
  if p_approved and exists(select 1 from public.nest_recurring_state_receipts
    ...
    perform private.nest_resume_recurring(p_household,p_operation,p_payload,p_approval);
    return private.nest_read_recurring_resume_approval(p_household,p_approval);
  end if;
  ...
  if not p_approved and v_row.status='denied' then return private.nest_read_recurring_resume_approval(p_household,p_approval); end if;
  perform private.nest_decide_action(p_approval,p_operation,v_command,1,p_payload,p_approved);
  if p_approved then perform private.nest_resume_recurring(p_household,p_operation,p_payload,p_approval); end if;
  return private.nest_read_recurring_resume_approval(p_household,p_approval);
end;
$$;
revoke all on function private.nest_decide_recurring_resume(uuid,uuid,jsonb,uuid,boolean) from public,anon,authenticated;
grant execute on function private.nest_decide_recurring_resume(uuid,uuid,jsonb,uuid,boolean) to authenticated;
create function public.nest_decide_recurring_resume(p_household uuid,p_operation uuid,p_payload jsonb,p_approval uuid,p_approved boolean)
returns jsonb language sql security invoker set search_path='' as $$
  select private.nest_decide_recurring_resume($1,$2,$3,$4,$5);
$$;
revoke all on function public.nest_decide_recurring_resume(uuid,uuid,jsonb,uuid,boolean) from public,anon,authenticated;
grant execute on function public.nest_decide_recurring_resume(uuid,uuid,jsonb,uuid,boolean) to authenticated;

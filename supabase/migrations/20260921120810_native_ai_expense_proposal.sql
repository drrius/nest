-- GATED: private expense proposals only. Models cannot approve, deny or post.
create function private.nest_expense_uuid(p_value jsonb,p_nullable boolean)
returns text language plpgsql immutable set search_path='' as $$
begin
  if p_nullable and p_value='null'::jsonb then return null; end if;
  if jsonb_typeof(p_value) is distinct from 'string' or p_value #>> '{}' !~* '^([0-9a-f]{8}-[0-9a-f]{4}-[1-8][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}|00000000-0000-0000-0000-000000000000|ffffffff-ffff-ffff-ffff-ffffffffffff)$' then
    raise exception 'Invalid expense identity' using errcode='22023';
  end if;
  return lower(p_value #>> '{}');
end;
$$;
revoke all on function private.nest_expense_uuid(jsonb,boolean) from public,anon,authenticated;

create function private.nest_canonical_expense(p_input jsonb)
...
begin
  if p_input is null or jsonb_typeof(p_input) is distinct from 'object' or octet_length(p_input::text)>32768
    ...
    or p_input-array['description','amountCentimes','payerId','allocations','date','note','categoryId']<>'{}'::jsonb
...
create function private.nest_propose_expense(p_household uuid,p_operation uuid,p_input jsonb)
returns jsonb language plpgsql security definer set search_path='' as $$
declare v_payload jsonb; v_approval uuid; v_members integer;
begin
  ...
  v_payload:=private.nest_canonical_expense(p_input);
  perform private.nest_expense_payload(v_payload,p_household);
  v_approval:=private.nest_propose_action(p_household,p_operation,'expenses.record',1,v_payload);
  return private.nest_read_expense_approval(p_household,v_approval);
end;
$$;
revoke all on function private.nest_propose_expense(uuid,uuid,jsonb) from public,anon,authenticated;
...
alter table public.nest_ai_commands add constraint nest_ai_commands_tool_name_check
  check(tool_name in ('proposeExpense','generateMealProposal','replaceProposalMeal','chooseProposalRecipe','discardMealProposal','editMealPreparation','createMealPreparation','placeLeftovers','placeRecipe','replaceWithRecipe','editRecipe','archiveRecipe','createRecipe','replaceMeal','moveMeal','removeMeal','placeMeal','completeChore','addGrocery','editGrocery','removeGrocery','checkGrocery',

-- GATED additive candidate. Recipe archive and private AI journal results commit atomically.
...
alter table public.nest_ai_commands add constraint nest_ai_commands_tool_name_check
  check(tool_name in ('archiveRecipe','createRecipe','replaceMeal','moveMeal','removeMeal','placeMeal','completeChore','addGrocery','editGrocery','removeGrocery','checkGrocery',
...
begin
  ...
  if p_tool='createRecipe' then
    return private.nest_create_recipe(p_household,p_operation,p_input);
  end if;
  if p_tool='replaceMeal' then

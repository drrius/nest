...
-- Internal bounded candidate read. Delivery must recheck these conditions when claiming/sending.
create function private.nest_renewal_reminder_candidates(p_from timestamptz,p_until timestamptz)
...
begin
  ...
  return query
    ...
    where d.due_at>=p_from and d.due_at<p_until
    order by d.due_at,r.household_id,r.id,m.user_id;
end;
$$;
revoke all on function private.nest_renewal_reminder_candidates(timestamptz,timestamptz) from public,anon,authenticated,service_role;
...
begin
  insert into private.nest_renewal_reminder_outbox(household_id,renewal_id,item_revision,schedule_revision,recipient_id,due_at)
    ...
    from private.nest_renewal_reminder_candidates(p_from,p_until) c
    where not exists(select 1 from private.nest_renewal_reminder_outbox o
      where (o.household_id,o.renewal_id,o.item_revision,o.schedule_revision,o.recipient_id,o.due_at)=
        (c.household_id,c.renewal_id,c.item_revision,c.schedule_revision,c.recipient_id,c.due_at))
    order by c.due_at,c.household_id,c.renewal_id,c.recipient_id limit 500

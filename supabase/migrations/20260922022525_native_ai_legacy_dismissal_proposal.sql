-- GATED: AI may propose dismissal only; native review and explicit decision remain mandatory.
create function private.nest_canonical_legacy_dismissal_proposal(p_input jsonb)
returns jsonb language plpgsql immutable security invoker set search_path='' as $$
begin
  if jsonb_typeof(p_input) is distinct from 'object' or octet_length(p_input::text)>1024
    or not (p_input ?& array['draftId']) or (p_input-array['draftId'])<>'{}'::jsonb
    or jsonb_typeof(p_input->'draftId') is distinct from 'string'
    or (p_input->>'draftId') !~* '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$' then
    raise exception 'Invalid legacy dismissal proposal' using errcode='22023'; end if;
  return jsonb_build_object('draftId',(p_input->>'draftId')::uuid);
end;
$$;
revoke all on function private.nest_canonical_legacy_dismissal_proposal(jsonb) from public,anon,authenticated,service_role;

create function private.nest_propose_legacy_dismissal(p_household uuid,p_operation uuid,p_input jsonb)
returns jsonb language plpgsql security definer set search_path='' as $$
declare v_actor uuid:=auth.uid(); v_draft public.expense_drafts; v_review jsonb; v_payload jsonb; v_approval uuid;
begin
  ...
  p_input:=private.nest_canonical_legacy_dismissal_proposal(p_input);
  ...
  if not found or v_draft.recurring_expense_rule_id is null then
    raise exception 'Draft unavailable' using errcode='42501'; end if;
  ...
  if v_draft.status<>'pending' or v_draft.source_kind<>'recurring' or v_review->'draft'->>'eventId' is not null then
    raise exception 'Draft needs reconciliation' using errcode='40001'; end if;
  v_payload:=jsonb_build_object('draftId',v_draft.id,'ruleId',v_draft.recurring_expense_rule_id,'reviewToken',v_review->>'reviewToken');
  v_approval:=private.nest_propose_action(p_household,p_operation,'recurring.dismiss-legacy-draft',1,v_payload);
  return private.nest_read_legacy_dismissal_approval(p_household,v_approval);
end;
$$;
revoke all on function private.nest_propose_legacy_dismissal(uuid,uuid,jsonb) from public,anon,authenticated,service_role;

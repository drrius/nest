-- GATED: private proposals only. Models cannot approve or execute recurring writes.
create function private.nest_canonical_recurring_proposal(p_input jsonb,p_new_rule uuid)
returns jsonb language plpgsql immutable set search_path='' as $$
declare v_input jsonb; v_config jsonb; v_shares jsonb;
begin
  if p_input is null or jsonb_typeof(p_input) is distinct from 'object'
    or (p_input->'ruleId'='null'::jsonb) is distinct from (p_input->'expectedRevision'='null'::jsonb) then
    raise exception 'Invalid recurring proposal identity' using errcode='22023'; end if;
  v_input:=p_input||jsonb_build_object('ruleId',case when p_input->'ruleId'='null'::jsonb then p_new_rule::text
    else private.nest_expense_uuid(p_input->'ruleId',false) end,
    'expectedRevision',private.nest_expense_uuid(p_input->'expectedRevision',true));
  perform private.nest_recurring_input(v_input);
  v_config:=v_input->'configuration';
  if v_config->>'mode'='fixed' then
    if jsonb_typeof(v_config->'allocations') is distinct from 'array' then
      raise exception 'Invalid recurring allocations' using errcode='22023'; end if;
    select jsonb_agg(share||jsonb_build_object('memberId',private.nest_expense_uuid(share->'memberId',false)) order by position)
      into v_shares from jsonb_array_elements(v_config->'allocations') with ordinality as shares(share,position);
    v_config:=v_config||jsonb_build_object('allocations',v_shares);
  end if;
  v_config:=v_config||jsonb_build_object('payerId',private.nest_expense_uuid(v_config->'payerId',false),
    'categoryId',private.nest_expense_uuid(v_config->'categoryId',true));
  return v_input||jsonb_build_object('configuration',v_config);
end;
$$;
revoke all on function private.nest_canonical_recurring_proposal(jsonb,uuid) from public,anon,authenticated;

create function private.nest_propose_recurring(p_household uuid,p_operation uuid,p_input jsonb)
returns jsonb language plpgsql security definer set search_path='' as $$
declare v_input jsonb; v_rule public.nest_recurring_rules; v_execution private.nest_recurring_execution;
  v_id uuid; v_members integer; v_today date; v_cycle jsonb; v_approval uuid;
begin
  ...
  v_input:=private.nest_canonical_recurring_proposal(p_input,gen_random_uuid());
  ...
  if v_rule.revision is distinct from (v_input->>'expectedRevision')::uuid or v_rule.status='cancelled' then
  ...
  perform private.nest_recurring_configuration(p_household,v_input->'configuration');
  ...
  if (v_input->'configuration'->>'startDate')::date<v_today then
    raise exception 'Recurring activation must be prospective' using errcode='22023'; end if;
  v_cycle:=private.nest_recurring_cycle(v_input->'configuration'->'schedule',
    (v_input->'configuration'->>'startDate')::date,v_execution.covered_through);
  if v_cycle is null or v_cycle->>'dueOn' is distinct from v_input->>'firstDueOn' then
    raise exception 'First recurring cycle changed' using errcode='40001'; end if;
  v_approval:=private.nest_propose_action(p_household,p_operation,
    case when v_rule.id is null then 'recurring.create' else 'recurring.update' end,1,v_input);
  return private.nest_read_recurring_approval(p_household,v_approval);
end;
$$;
revoke all on function private.nest_propose_recurring(uuid,uuid,jsonb) from public,anon,authenticated;

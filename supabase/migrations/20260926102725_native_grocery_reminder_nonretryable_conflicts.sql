-- Stale grocery reminder baselines require a fresh review.
-- Preserve existing lock contention handlers and immutable operation receipts.
do $patch$
declare v_target record; v_definition text; v_body text; v_tail text;
begin
  for v_target in select * from (values
    ('private.nest_save_grocery_reminder(uuid,uuid,jsonb)',3),
    ('private.nest_read_grocery_reminder(uuid,uuid)',1)
    ...
    if (length(v_body)-length(replace(v_body,
      ...
      raise exception 'Unexpected reminder conflict definition: %',v_target.signature;
    end if;

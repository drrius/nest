-- GATED: private variable-cycle proposals only. Model tools never approve or post money.
create function private.nest_canonical_variable_cycle_proposal(p_input jsonb)
returns jsonb language plpgsql immutable set search_path='' as $$
declare v_input jsonb; v_share jsonb; v_allocations jsonb:='[]';
begin
  if p_input is null or jsonb_typeof(p_input) is distinct from 'object'
    ...
    raise exception 'Invalid variable cycle proposal' using errcode='22023'; end if;
  ...
  perform private.nest_variable_cycle_input(v_input);
  if jsonb_array_length(p_input->'allocations')<>2 then raise exception 'Invalid allocations' using errcode='22023'; end if;
  for v_share in select value from jsonb_array_elements(p_input->'allocations') loop
    if jsonb_typeof(v_share) is distinct from 'object' or not v_share ?& array['memberId','centimes']
      or v_share-array['memberId','centimes']<>'{}'::jsonb then
      raise exception 'Invalid allocation' using errcode='22023'; end if;
    v_allocations:=v_allocations||jsonb_build_array(v_share||jsonb_build_object('memberId',private.nest_expense_uuid(v_share->'memberId',false)));
  end loop;
  return v_input||jsonb_build_object('allocations',v_allocations);
end;
$$;
revoke all on function private.nest_canonical_variable_cycle_proposal(jsonb) from public,anon,authenticated;
create function private.nest_propose_variable_cycle(p_household uuid,p_operation uuid,p_input jsonb)
returns jsonb language plpgsql security definer set search_path='' as $$
declare v_input jsonb; v_rule public.nest_recurring_rules; v_id uuid; v_members integer; v_approval uuid;
  v_cursor private.nest_recurring_execution; v_due date; v_cycle jsonb; v_expense jsonb;
begin
  ...
  if v_members<>2 then raise exception 'Variable proposal requires two members' using errcode='23514'; end if;
  v_input:=private.nest_canonical_variable_cycle_proposal(p_input);
  v_id:=(v_input->>'ruleId')::uuid; v_due:=(v_input->>'dueOn')::date;
  ...
  if not found or v_rule.revision is distinct from (v_input->>'expectedRevision')::uuid
    or v_rule.status<>'active' or v_rule.configuration->>'mode'<>'variable' then
  ...
  if v_cycle is null or v_cycle->>'dueOn' is distinct from v_input->>'dueOn'
  ...
  v_expense:=(v_rule.configuration-array['startDate','schedule','mode'])||jsonb_build_object(
    'date',v_input->>'dueOn','amountCentimes',v_input->'amountCentimes','allocations',v_input->'allocations');
  perform private.nest_expense_payload(v_expense,p_household);
  v_approval:=private.nest_propose_action(p_household,p_operation,'recurring.record-cycle',1,v_input);
  return private.nest_read_variable_cycle_approval(p_household,v_approval);
end;
$$;
revoke all on function private.nest_propose_variable_cycle(uuid,uuid,jsonb) from public,anon,authenticated;

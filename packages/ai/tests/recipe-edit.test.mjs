...
test("edit tool exposes only a strict recipe target and library baseline", async () => {
  const adapter = effectSchema(AssistantInputs.editRecipe),
  ...
  for (const patch of [
    ...
    { householdId: "hidden" },
    { patch: { unknown: "value" } },
    { ingredients: [{ kind: "existing", ingredientId: "invalid", patch: {} }] },
    { expectedRevision: "01" },

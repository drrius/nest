...
export function recurringReminderRoute(
  request: Request,
  ...
  return Effect.gen(function* () {
    const url = new URL(request.url),
      service = recurringReminderService(config, caller);
    if (request.method === "GET") {
      const key = url.pathname.endsWith("/operation") ? "operationId" : "ruleId";
      ...
      return yield* key === "operationId"
        ...
        : service.read({ ruleId: url.searchParams.get(key) });
    }

import * as Effect from "effect/Effect";
import * as Schema from "effect/Schema";
import {
  NotificationPreferences,
  NotificationPreferenceReceipt,
  SaveNotificationPreferences,
} from "@nest/contracts/notifications";
import { ConversationRevision } from "@nest/contracts/conversations";
import { ApiFailure } from "../errors.ts";
import { requestDocument, requestJson } from "../supabase-request.ts";
import type { AuthorizedCaller } from "../chores/service.ts";
import type { IdentityConfig } from "../supabase-identity.ts";
const Row = Schema.Struct({
  ...NotificationPreferences.fields,
  actorId: Schema.String,
  householdId: Schema.String,
  revision: ConversationRevision,
});
const decode = <A>(
  schema: Schema.Codec<A>,
  value: unknown,
  code: ApiFailure["code"] = "unavailable",
) =>
  Schema.decodeUnknownEffect(schema)(value, { onExcessProperty: "error" }).pipe(
    Effect.mapError(() => new ApiFailure({ code })),
  );
export function notificationPreferences(config: IdentityConfig, caller: AuthorizedCaller) {
  const { userId: actorId, householdId } = caller.member;
  return {
    read: () =>
      Effect.gen(function* () {
        const query = new URLSearchParams({
          select:
            "actorId:actor_id,householdId:household_id,revision:revision::text,dailySummaryEnabled:daily_summary_enabled,dailySummaryTime:daily_summary_time,itemRemindersEnabled:item_reminders_enabled",
          actor_id: `eq.${actorId}`,
          household_id: `eq.${householdId}`,
          limit: "1",
        });
        const document = yield* requestDocument(
          config,
          caller.token,
          `rest/v1/nest_notification_preferences?${query}`,
        );
        const rows = yield* decode(Schema.Array(Row), document.value);
        if (rows.length === 0 && document.range === "*/0") return null;
        const row = yield* decode(Row, rows[0]);
        if (
          rows.length !== 1 ||
          document.range !== "0-0/1" ||
          row.actorId !== actorId ||
          row.householdId !== householdId ||
          row.revision === "0"
        )
          return yield* new ApiFailure({ code: "unavailable" });
        return {
          revision: row.revision,
          preferences: {
            dailySummaryEnabled: row.dailySummaryEnabled,
            dailySummaryTime: row.dailySummaryTime,
            itemRemindersEnabled: row.itemRemindersEnabled,
          },
        };
      }),
    save: (input: unknown) =>
      Effect.gen(function* () {
        const command = yield* decode(SaveNotificationPreferences, input, "invalid_request");
        const prefs = command.preferences;
        const raw = yield* requestJson(
          config,
          caller.token,
          "rest/v1/rpc/nest_save_notification_preferences",
          {
            p_household: householdId,
            p_operation: command.operationId.toLowerCase(),
            p_expected: command.expectedRevision,
            p_daily_enabled: prefs.dailySummaryEnabled,
            p_daily_time: prefs.dailySummaryTime,
            p_items_enabled: prefs.itemRemindersEnabled,
          },
        );
        const receipt = yield* decode(NotificationPreferenceReceipt, raw);
        if (
          receipt.actorId !== actorId ||
          receipt.householdId !== householdId ||
          receipt.operationId !== command.operationId.toLowerCase() ||
          receipt.revision !== String(BigInt(command.expectedRevision) + 1n)
        )
          return yield* new ApiFailure({ code: "unavailable" });
        return receipt;
      }),
  };
}

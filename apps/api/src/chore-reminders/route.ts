...
export function choreReminderRoute(
  request: Request,
  ...
  return Effect.gen(function* () {
    const url = new URL(request.url),
      service = choreReminderService(config, caller);
    if (request.method === "GET") {
      const key = url.pathname.endsWith("/operation") ? "operationId" : "occurrenceId";
      ...
      return yield* key === "operationId"
        ...
        : service.read({ occurrenceId: url.searchParams.get(key) });
    }

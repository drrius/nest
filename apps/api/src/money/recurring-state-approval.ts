...
export function recurringStateApprovals(config: IdentityConfig, caller: AuthorizedCaller) {
  const validate = (raw: unknown, approvalId: string) =>
    Effect.gen(function* () {
      const result = yield* decode(RecurringStateApprovalEnvelope, raw, "unavailable");
      if (
  ...
  return {
    read: (input: unknown) =>
      Effect.gen(function* () {
        const query = yield* decode(RecurringStateApprovalQuery, input, "invalid_request");
        const approvalId = query.approvalId.toLowerCase();
        return yield* validate(
          yield* requestJson(
            config,
            caller.token,
            "rest/v1/rpc/nest_read_recurring_state_approval",
            {
    ...
    decide: (input: unknown) =>
      Effect.gen(function* () {
        const command = yield* decode(DecideRecurringState, input, "invalid_request");
        ...
        const result = yield* validate(
          yield* requestJson(config, caller.token, "rest/v1/rpc/nest_decide_recurring_state", {
            p_household: caller.member.householdId,

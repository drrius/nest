...
export function financialProposalTools<T>(
  ...
  return {
    ...
    proposeExpense: write(
      "proposeExpense",
      "Propose only an expense the member explicitly asked to record. Read household member IDs first; never invent them. Ask for missing amount, payer, date or split. Use exact CHF centime strings and exactly two member allocations summing to the amount; equal splits assign an odd centime to the payer. Category is null unless a current category ID is known; note is null when absent. This creates a private pending proposal and posts no money. Tell the member to open the expense approval and confirm its exact amount, payer and allocations. Never claim it is posted, infer consent from conversation, or try to approve or execute it yourself. For an explicitly requested grocery expense, ask separately for the receipt total and shared amount. Set receiptTotalCentimes to the explicit total and amountCentimes to the shared amount, with allocations summing only to that shared amount. The shared amount cannot exceed the total. Omit receiptTotalCentimes for ordinary expenses. Checking groceries never implies an expense or provides either amount. Receipt selection requires a native handoff and is not included in this proposal. To attach a new receipt, direct the member to the native Record expense or Grocery expense form, choose a photo or PDF, upload it, then review and confirm there. You cannot select, read or upload device files. Never invent a receipt path or claim that a file was uploaded.",
    ),
  };
}

...
import {
  RecurringStateSaveQuery,
  RecurringStateSaveResult,
} from "@nest/contracts/recurring-state-read";
import type { AuthorizedCaller } from "../chores/service.ts";
import type { IdentityConfig } from "../supabase-identity.ts";
import { ApiFailure } from "../errors.ts";
import { requestJson } from "../supabase-request.ts";
export function recurringStateRecovery(config: IdentityConfig, caller: AuthorizedCaller) {
  const run = (input: unknown, cancel: boolean) =>
    Effect.gen(function* () {
      const query = yield* Schema.decodeUnknownEffect(RecurringStateSaveQuery)(input, {
      ...
      const result = yield* Schema.decodeUnknownEffect(RecurringStateSaveResult)(raw, {
        onExcessProperty: "error",

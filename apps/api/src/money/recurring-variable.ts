...
export function variableCycleCommands(config: IdentityConfig, caller: AuthorizedCaller) {
  const run = (input: unknown, approved: boolean) =>
    Effect.gen(function* () {
      const command = yield* Schema.decodeUnknownEffect(
        approved ? ExecuteVariableCycle : SaveVariableCycle,
      )(input, {
        onExcessProperty: "error",
      }).pipe(Effect.mapError(() => new ApiFailure({ code: "invalid_request" })));
      const approvalId = Schema.is(ExecuteVariableCycle)(command)
      ...
      const raw = yield* requestJson(
        ...
        caller.token,
        approved
          ? "rest/v1/rpc/nest_execute_variable_cycle"
          : "rest/v1/rpc/nest_save_variable_cycle",
        {
      ...
      const result = yield* Schema.decodeUnknownEffect(VariableCycleReceipt)(raw, {
        onExcessProperty: "error",

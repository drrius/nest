...
export function recurringStateCommands(config: IdentityConfig, caller: AuthorizedCaller) {
  const run = (input: unknown, approved: boolean) =>
    Effect.gen(function* () {
      const command = yield* Schema.decodeUnknownEffect(
        approved ? ExecuteRecurringState : SaveRecurringState,
      )(input, {
        onExcessProperty: "error",
      }).pipe(Effect.mapError(() => new ApiFailure({ code: "invalid_request" })));
      const approvalId = Schema.is(ExecuteRecurringState)(command)
      ...
      const raw = yield* requestJson(
        ...
        caller.token,
        approved
          ? "rest/v1/rpc/nest_execute_recurring_state"
          : "rest/v1/rpc/nest_save_recurring_state",
        {
      ...
      const result = yield* Schema.decodeUnknownEffect(RecurringStateReceipt)(raw, {
        onExcessProperty: "error",

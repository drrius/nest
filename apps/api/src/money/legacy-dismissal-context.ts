...
export function legacyDismissalContext(
  config: IdentityConfig,
  caller: AuthorizedCaller,
  input: unknown,
) {
  return Effect.gen(function* () {
    const query = yield* Schema.decodeUnknownEffect(LegacyDismissalApprovalQuery)(input, {
    ...
    const raw = yield* requestJson(
      config,
      caller.token,
      "rest/v1/rpc/nest_read_legacy_dismissal_context",
      { p_household: caller.member.householdId, p_approval: approvalId },
    );
    const result = yield* Schema.decodeUnknownEffect(LegacyDismissalContext)(raw, {
      onExcessProperty: "error",

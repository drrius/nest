...
export function legacyAdoptionApprovals(config: IdentityConfig, caller: AuthorizedCaller) {
  const validate = (raw: unknown, approvalId: string) =>
    Effect.gen(function* () {
      const result = yield* decode(LegacyAdoptionApprovalEnvelope, raw, "unavailable");
      if (
  ...
  return {
    read: (input: unknown) =>
      Effect.gen(function* () {
        const query = yield* decode(LegacyAdoptionApprovalQuery, input, "invalid_request");
        const approvalId = query.approvalId.toLowerCase();
        return yield* validate(
          yield* requestJson(
            config,
            caller.token,
            "rest/v1/rpc/nest_read_legacy_adoption_approval",
            {
    ...
    decide: (input: unknown) =>
      Effect.gen(function* () {
        const command = yield* decode(DecideLegacyAdoption, input, "invalid_request");
        ...
        const result = yield* validate(
          yield* requestJson(config, caller.token, "rest/v1/rpc/nest_decide_legacy_adoption", {
            p_household: caller.member.householdId,

...
export function legacyDismissalRecovery(config: IdentityConfig, caller: AuthorizedCaller) {
  const run = (input: unknown, cancel: boolean) =>
    Effect.gen(function* () {
      const query = yield* Schema.decodeUnknownEffect(LegacyDismissalRecoveryQuery)(input, {
      ...
      const raw = yield* requestJson(
        ...
        caller.token,
        cancel
          ? "rest/v1/rpc/nest_cancel_legacy_dismissal"
          : "rest/v1/rpc/nest_read_legacy_dismissal",
        {
          p_household: caller.member.householdId,
          p_operation: operationId,
        },
      );
      const result = yield* Schema.decodeUnknownEffect(LegacyDismissalRecovery)(raw, {
        onExcessProperty: "error",

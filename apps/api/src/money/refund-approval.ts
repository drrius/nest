...
export function refundApprovals(config: IdentityConfig, caller: AuthorizedCaller) {
  const validate = (raw: unknown, approvalId: string) =>
    Effect.gen(function* () {
      const result = yield* decode(RefundApprovalEnvelope, raw, "unavailable");
      if (
  ...
  return {
    read: (input: unknown) =>
      Effect.gen(function* () {
        const query = yield* decode(RefundApprovalQuery, input, "invalid_request");
        const approvalId = query.approvalId.toLowerCase();
        return yield* validate(
          yield* requestJson(config, caller.token, "rest/v1/rpc/nest_read_refund_approval", {
    ...
    decide: (input: unknown) =>
      Effect.gen(function* () {
        const command = yield* decode(DecideRefund, input, "invalid_request");
        ...
        const result = yield* validate(
          yield* requestJson(config, caller.token, "rest/v1/rpc/nest_decide_refund", {
            p_household: caller.member.householdId,
        ...
        if (
          ...
          !equivalent(result.approval.refund, refund) ||

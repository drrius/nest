import { variableCycleApprovals } from "./recurring-variable-approval.ts";
import * as Effect from "effect/Effect";
import { ApiFailure } from "../errors.ts";
import type { AuthorizedCaller } from "../chores/service.ts";
import type { IdentityConfig } from "../supabase-identity.ts";
import { commandBody } from "../request-body.ts";
export function variableCycleApprovalRoute(
  request: Request,
  ...
  return Effect.gen(function* () {
    ...
    const commands = variableCycleApprovals(config, caller);
    if (url.pathname === "/v1/money/recurring/variable/approval") {

...
export function legacyConfirmationCommands(config: IdentityConfig, caller: AuthorizedCaller) {
  const run = (input: unknown, approved: boolean) =>
    Effect.gen(function* () {
      const command = yield* Schema.decodeUnknownEffect(
        approved ? ExecuteLegacyConfirmation : SaveLegacyConfirmation,
      )(input, {
        onExcessProperty: "error",
      }).pipe(Effect.mapError(() => new ApiFailure({ code: "invalid_request" })));
      const approvalId = Schema.is(ExecuteLegacyConfirmation)(command)
      ...
      const raw = yield* requestJson(
        ...
        caller.token,
        approved
          ? "rest/v1/rpc/nest_execute_legacy_confirmation"
          : "rest/v1/rpc/nest_save_legacy_confirmation",
        {
      ...
      const result = yield* Schema.decodeUnknownEffect(LegacyConfirmationReceipt)(raw, {
        onExcessProperty: "error",

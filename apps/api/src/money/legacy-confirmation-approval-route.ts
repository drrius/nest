import { legacyConfirmationContext } from "./legacy-confirmation-context.ts";
import { legacyConfirmationApprovals } from "./legacy-confirmation-approval.ts";
import * as Effect from "effect/Effect";
import { ApiFailure } from "../errors.ts";
import type { AuthorizedCaller } from "../chores/service.ts";
import type { IdentityConfig } from "../supabase-identity.ts";
import { commandBody } from "../request-body.ts";
export function legacyConfirmationApprovalRoute(
  request: Request,
  ...
  return Effect.gen(function* () {
    ...
    const commands = legacyConfirmationApprovals(config, caller);
    if (request.method === "GET") {
      if (params.size !== 1 || !params.has("approvalId"))
        return yield* new ApiFailure({ code: "invalid_request" });
      if (url.pathname.endsWith("/context"))
        return yield* legacyConfirmationContext(config, caller, {

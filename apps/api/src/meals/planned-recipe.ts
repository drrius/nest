...
export function readPlannedRecipe(
  ...
  return Effect.gen(function* () {
    ...
    const raw = yield* requestJson(config, caller.token, "rest/v1/rpc/nest_planned_recipe", {
      p_household: caller.member.householdId,
      p_week: query.weekStart,
      p_revision: query.revision,
      p_entry: query.entryId.toLowerCase(),
    });
    const result = yield* Schema.decodeUnknownEffect(PlannedRecipeEnvelope)(raw, {
      onExcessProperty: "error",
...
export function plannedRecipeRoute(
  request: Request,
  config: IdentityConfig,
  caller: AuthorizedCaller,
) {
  ...
  return readPlannedRecipe(config, caller, {

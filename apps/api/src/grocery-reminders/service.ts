...
export function groceryReminderService(config: IdentityConfig, caller: AuthorizedCaller) {
  ...
  const change = (input: unknown) =>
    Effect.gen(function* () {
      const command = canonicalGroceryReminder(
        yield* decode(SaveGroceryReminder, input, "invalid_request"),
      );
      const { operationId, ...p_input } = command;
      const result = yield* rpc(GroceryReminderReceipt, "nest_save_grocery_reminder", {
        p_operation: operationId,
        p_input,
      });
      if (!sameGroceryReminderCommand(result.command, command))
        return yield* new ApiFailure({ code: "unavailable" });
      return result;
    });
  return {
    ...
    read: (input: unknown) =>
      Effect.gen(function* () {
        const query = yield* decode(GroceryReminderQuery, input, "invalid_request"),
          id = query.itemId.toLowerCase();
        const result = yield* rpc(GroceryReminderContext, "nest_read_grocery_reminder", {
          p_item: id,
        });
        if (result.grocery.itemId !== id) return yield* new ApiFailure({ code: "unavailable" });
        return result;
      }),
    recover: (input: unknown, cancel: boolean) =>
      Effect.gen(function* () {
        ...
        const result = yield* rpc(
          GroceryReminderRecovery,
          cancel
            ? "nest_cancel_grocery_reminder_operation"
            : "nest_read_grocery_reminder_operation",
          { p_operation: operationId },
        );

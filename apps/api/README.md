...
Copy `.env.example` to `.env` in this directory and configure an isolated development backend whose audited legacy prerequisites and gated Nest migrations have been deliberately prepared. Run `pnpm --filter @nest/api dev` with Node 24+. The Node HTTP adapter binds **127.0.0.1 only** on port 8787; it propagates disconnect cancellation, request streaming and no-store responses. Startup performs no migration or deployment. Private meal generation optionally uses the separately configured server-only planning secret described below.
...
## Private meal proposal generation

`POST /v1/meals/proposal/generate` accepts `operationId`, `weekStart`, `expectedWeekRevision` and `familiarOnly`. It returns an immutable start receipt plus the current private proposal envelope. Retries reuse the operation ID; generating means pending, never saved meals. `GET /v1/meals/proposal?proposalId=…` reads it, `POST /v1/meals/proposal/recover` with `proposalId` fails expired unfinished work, and `POST /v1/meals/proposal/discard` requires `operationId`, `proposalId` and `expectedRevision`. These owner-only operations do not approve meals or add groceries.

Generation requires the existing explicit Gateway configuration plus `NEST_SUPABASE_PLANNING_SECRET=sb_secret_…` on the server. Never use this secret in mobile configuration. Missing generation configuration returns 503; read/recovery/discard still work. The finite privileged transport uses only this secret, while authorization and ordinary reads use the user's bearer and publishable key. No default model, hosted credential, deployment or production migration is supplied. See the [planning boundary](../../docs/native-rewrite/meal-planning-boundary.md) for deadline, privacy, bounded-shortlist and live-provider verification limits.

Native clients can first call `POST /v1/meals/proposal/reserve` with the same generation input to obtain the immutable start receipt without invoking a model. Persist that proposal ID and the original operation before calling `/generate`. Reservation/recovery are distinct from approval and do not write the active week.

`POST /v1/meals/proposal/approve` is the explicit native approval endpoint. Supply `operationId`, `proposalId` and the exact displayed `expectedRevision`; no recipe or approval flag is accepted. It atomically saves the reviewed plan only when current constraints and the week baseline match. Retain the same operation when a response is lost: its historical receipt is replayed without re-posting, even after later week edits. The response contains `{ version: 1, receipt }` with posted entry mappings. This endpoint uses caller authentication and works without model/planning credentials. Approval adds no groceries; native confirmation/recovery is implemented but remains device-unverified.

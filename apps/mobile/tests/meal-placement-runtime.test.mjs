...
import { mealPlacementOwner } from "../src/meals/placement-owner.ts";
import { PreferenceFailure } from "../src/preferences/client.ts";
const id = (n) => `00000000-0000-4000-8000-${String(n).padStart(12, "0")}`;
const target = { weekStart: "2026-10-05", date: "2026-10-06", slot: "lunch" };
...
test("uncertain placement freezes the exact baseline, title and operation until retry", async () => {
  const calls = [];
  let current = snapshot(),
    response = failure(),
    reads = 0;
  const runtime = new MealPlacementRuntime(
    {
      read: () => {
        reads++;
...
test("acknowledged placement reloads without resending after refresh failure or an older reply", async () => {
...
test("conflicts require explicit reload; occupied slots and invalid drafts cannot dispatch", async () => {
  /** @type {Effect.Effect<ReturnType<typeof snapshot>, PreferenceFailure>} */
  let read = Effect.succeed(snapshot());
  let writes = 0;
  const runtime = new MealPlacementRuntime(
  ...
  await runtime.retry();
  assert.equal(writes, 1);
  read = Effect.succeed(snapshot("1", [{ ...target, title: "Partner meal" }]));
  await runtime.load();
  await runtime.save("Replacement");
  assert.match(runtime.getSnapshot().notice, /already has a meal/);
  assert.equal(writes, 1);
  runtime.dispose();
});
...
test("access denial hides data and remains sticky until an authorized read succeeds", async () => {
  ...
  let read = Effect.succeed(snapshot());
  const runtime = new MealPlacementRuntime(
    { read: () => read, place: () => failure("forbidden") },
    target,
    uuid,
  );
  await runtime.load();
...
test("Strict Mode owner release recreates a usable runtime and initial reads are not pending writes", async () => {
  ...
  const client = {
    ...
    place: () => Effect.succeed(receipt),
  };
  const owner = mealPlacementOwner(client, target, uuid);

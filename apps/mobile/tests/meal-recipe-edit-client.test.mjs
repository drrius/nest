...
const run = (body, fetch) =>
  Effect.runPromise(
    client.editRecipe(body).pipe(Effect.provideService(FetchHttpClient.Fetch, fetch)),
  );

test("native edit sends authenticated exact draft and rejects substituted receipts", async () => {
  assert.deepEqual(
    await run(command, async (url, init) => {
      assert.equal(new URL(url).pathname, "/v1/meals/recipe/edit");

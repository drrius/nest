...
import {
  ...
  id,
  account,
} from "./money-settlement-approval-fixture.mjs";
...
test("settlement recovery persists before dispatch and reopens committed outcome without another write", async (t) => {
  ...
  f.behavior(() =>
    Effect.gen(function* () {
      assert.deepEqual(yield* f.db.store.readSettlementApproval(f.db.session, pending.id), intent);
      f.current(consumed());
  ...
  const next = new SettlementApprovalRuntime(
    settlementApprovalOperations({ store: reopened.store, session: f.db.session }, f.client),
    pending.id,
  );
  ...
  assert.equal(f.calls(), 1);
  assert.equal(await run(reopened.store.readSettlementApproval(f.db.session, pending.id)), null);
  next.dispose();
});
test("unresolved decision survives reload and only explicit exact retry can send", async (t) => {
  ...
  assert.equal(await run(f.db.store.readSettlementApproval(f.db.session, pending.id)), null);
  runtime.dispose();
});
test("stale alert and expired initial decision cannot dispatch; saved exact retry remains recoverable", async (t) => {
  ...
  await runtime.decide(runtime.getSnapshot().approval, true);
  assert.equal(f.calls(), 0);
  await run(f.db.store.stageSettlementApproval(f.db.session, intent, () => true));
  await runtime.refresh();
  await runtime.retry();
  assert.equal(f.calls(), 1);
  runtime.dispose();
...
test("conflicting persisted decision and storage failure stop before any financial dispatch", async (t) => {
  ...
  await runtime.setActive(true);
  await run(
    f.db.store.stageSettlementApproval(f.db.session, { ...intent, approved: false }, () => true),
  );
  await runtime.decide(runtime.getSnapshot().approval, true);
  assert.equal(f.calls(), 0);
  assert.equal(runtime.getSnapshot().fresh, false);
  ...
  assert.equal(runtime.getSnapshot().attempt.approved, false);
  runtime.dispose();
  const broken = new SettlementApprovalRuntime(
    { ...f.operations, stage: () => unavailable() },
    pending.id,
    () => 1,
  );
  await run(f.db.store.clearSettlementApproval(f.db.session, { ...intent, approved: false }));
...
test("background cancellation retains staged decision and reconciles late commit without another write", async (t) => {
  ...
  await sending;
  assert.equal(runtime.getSnapshot().approval, null);
  assert.deepEqual(await run(f.db.store.readSettlementApproval(f.db.session, pending.id)), intent);
  await runtime.setOnline(true);
  await runtime.setActive(true);
...
test("saved operation mismatch prevents recovery from substituting another approval command", async (t) => {
  const f = await fixture(t);
  await run(
    f.db.store.stageSettlementApproval(
      f.db.session,

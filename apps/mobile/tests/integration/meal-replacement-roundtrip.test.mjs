...
const target = { weekStart: "2026-09-21", entryId: id(100), date: "2026-09-22", slot: "lunch" };
async function backend(t, lossy = false) {
  ...
  const runtime = new MealReplacementRuntime(client, target, () => id(200));
  t.after(() => runtime.dispose());
  return { remote, proxy, client, account, runtime };
}

test("native replacement retries lost acknowledgment after the partner edits the new meal and refreshes the actual SQLite week", async (t) => {
  const f = await backend(t, true);
  await f.runtime.load();
  await f.runtime.save("Lentil soup");
  ...
  f.remote.db.sql(
    `update public.meal_plan_entries set title_snapshot='Partner soup' where id='${saved.entryId}'`,
  );
  await f.runtime.save("Lentil soup");
  await f.runtime.retry();
  assert.equal(f.runtime.getSnapshot().stage, "saved");
  assert.deepEqual(f.runtime.getSnapshot().receipt, saved);
  assert.equal(f.runtime.getSnapshot().snapshot.revision, "4");
...
test("native replacement conflicts on partner edits and hides data after revoked reads", async (t) => {

...
const target = { sourceWeekStart: "2026-09-21", entryId: id(100) };
async function backend(t, lossy = false) {
  ...
  const runtime = new MealMoveRuntime(client, target, () => id(200));
  t.after(() => runtime.dispose());
  return { remote, proxy, client, account, runtime };
}

test("native move retries lost acknowledgment after a partner moves the meal again and refreshes the actual SQLite week", async (t) => {
  const f = await backend(t, true);
  await f.runtime.load();
  await f.runtime.selectWeek("2026-09-28");
  await f.runtime.save({ date: "2026-09-29", slot: "dinner" });
  assert.equal(f.runtime.getSnapshot().stage, "uncertain");
  assert.equal(f.proxy.dropped(), 1);
  const saved = JSON.parse(f.remote.db.sql("select result from public.nest_meal_move_receipts"));
  f.remote.db.sql(
    `update public.meal_plan_entries set date='2026-10-06' where id='${saved.entryId}'`,
  );
  await f.runtime.save({ date: "2026-09-29", slot: "dinner" });
  await f.runtime.retry();
  assert.equal(f.runtime.getSnapshot().stage, "saved");
  assert.deepEqual(f.runtime.getSnapshot().receipt, saved);
  assert.equal(f.runtime.getSnapshot().source.revision, "2");
  ...
  const viewer = new MealWeekRuntime(
    f.client,
    { store: local.store, session },
    target.sourceWeekStart,
  );
  ...
  assert.deepEqual(
    await run(local.store.readMealWeek(session, target.sourceWeekStart)),
    f.runtime.getSnapshot().source,
  );
  assert.deepEqual((await run(local.store.read(session))).pending, []);
});

test("native move conflicts on partner edits and hides data after revoked reads", async (t) => {
  ...
  await f.runtime.load();
  assert.equal(f.runtime.getSnapshot().source.entries[0].title, "Partner changed meal");
  f.remote.db.sql(`delete from public.household_members where user_id='${id(1)}'`);
  await f.runtime.save({ date: "2026-09-29", slot: "dinner" });
  assert.equal(f.runtime.getSnapshot().stage, "verify");
  assert.equal(f.runtime.getSnapshot().source, null);
});

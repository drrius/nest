...
test("native edit recovers an exact lost-response attempt after partner work and handles stale and revoked access", async (t) => {
  ...
  await runtime.edit(original, { title: "First edit" });
  assert.equal(proxy.dropped(), 1);
  assert.equal(runtime.getSnapshot().stage, "uncertain");
  await partner.load();
  await partner.edit(partner.getSnapshot().snapshot.routines[0], { title: "Partner edit" });
  assert.equal(partner.getSnapshot().stage, "ready");
  await runtime.retry();
  assert.equal(runtime.getSnapshot().stage, "ready");
  assert.equal(runtime.getSnapshot().snapshot.routines[0].definition.title, "Partner edit");
  assert.equal(remote.db.sql("select count(*) from public.nest_routine_edit_receipts"), "2");
  await runtime.edit(original, { title: "Stale draft" });
  ...
  await runtime.edit(runtime.getSnapshot().snapshot.routines[0], { title: "Revoked" });

...
import {
  ...
  account,
  id,
} from "./money-recurring-state-save-fixture.mjs";
...
test("subscription owner recreates recovery after cleanup without stale lifecycle dispatch", async (t) => {
  const { recurringStateSaveOwner } = await import("../src/money/recurring-state-save-owner.ts");
  const f = await fixture(t),
    owner = recurringStateSaveOwner(f.operations);
...
test("returning from background retains the known terminal result until explicit acknowledgment", async (t) => {
  const f = await fixture(t),
...
test("connection loss interrupts pending work and reconnect reconciles without an automatic resend", async (t) => {
  ...
  assert.equal(runtime.getSnapshot().fresh, false);
  assert.deepEqual(await run(f.db.store.readRecurringStateSave(f.db.session)), attempt);

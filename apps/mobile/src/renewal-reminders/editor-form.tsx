...
function Form(props: Props & { initial: ReminderEditorContext }) {
  ...
  const submit = () => {
    ...
    const dialog = reminderConfirmation(
      {
        operationId,
        renewalId: initial.renewal.renewalId,
        expectedRenewalRevision: initial.renewal.revision,
...
function matching(context: Props["context"], initial: ReminderEditorContext) {
  ...
  if (!context.fresh || !value || value.renewal.removed) return false;
  return (
    value.renewal.revision === initial.renewal.revision &&

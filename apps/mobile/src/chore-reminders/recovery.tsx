...
export function ReminderRecovery({
  ...
  runtime: ChoreReminderSaveRuntime;
  occurrenceId: string;
  ...
  if (!view.active) return <Note>Chore details are hidden while inactive.</Note>;
  return (
    <RecoveryContent runtime={runtime} occurrenceId={occurrenceId} context={context} next={next} />
  );
}
function RecoveryContent({
  ...
  const review = recoveryReview(runtime, occurrenceId, context);
...
function Terminal({
  ...
  return (
    <>
      <Note>
        {recorded
          ? "Reminder settings saved. The chore is unchanged."
...
function recoveryReview(
  ...
  return command ? reminderRecoveryReview(command, occurrenceId, context) : null;
}
function OtherTarget({ target, disabled }: { target: string; disabled: boolean }) {
  ...
  return (
    <>
      <Note>
        An earlier reminder change belongs to another chore. Open that chore to review its settings
        and outcome.
      </Note>
      <NativeAction
        ...
        disabled={disabled}
        onPress={() =>
          router.replace({ pathname: "/chore-reminder", params: { occurrenceId: target } })
        }
      />
    </>
  );

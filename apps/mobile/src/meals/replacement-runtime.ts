...
import { ReplaceMeal, type MealReplacementReceipt } from "@nest/contracts/meal-replacement";
...
export type ReplacementTarget = Readonly<
  Pick<ReplaceMeal, "weekStart" | "date" | "slot" | "entryId">
>;
export type ReplacementView = {
  ...
  notice: string | null;
  receipt: MealReplacementReceipt | null;
};
const initial: ReplacementView = {
  snapshot: null,
  busy: false,
  pendingWrite: false,
  stage: "ready",
...
export class MealReplacementRuntime {
  private view = initial;
  private attempt: ReplaceMeal | null = null;
  ...
  private uuid: () => string;
  constructor(client: MealClient, target: ReplacementTarget, uuid: () => string) {
    this.client = client;
  ...
  private publish(patch: Partial<ReplacementView>) {
  ...
  private async read() {
    ...
    this.publish({
      ...
      notice: this.view.receipt
        ? "Your meal was replaced. The week now shows the current household plan."
        : null,
    });
  }
  load = async () => {
  ...
  save = async (title: string) => {
    ...
    if (
      !this.view.snapshot.entries.some(
        (entry) =>
          entry.entryId === this.target.entryId &&
          entry.date === this.target.date &&
          entry.slot === this.target.slot,
      )
    ) {
      this.publish({
        notice: "The original meal is no longer in this slot. Return to the current week.",
      });
      return;
    }
    const parsed = Schema.decodeUnknownExit(ReplaceMeal)(
      {
  ...
  private async send() {
    ...
    try {
      const receipt = await this.run(this.client.replace(this.attempt));
      if (this.disposed) return;
  ...
  private failed(error: unknown) {
    ...
    if (code === "session" || code === "forbidden") {
      ...
      this.publish({
        ...
        stage: "verify",
        pendingWrite: false,
        notice: "Verify your account before replacing meals.",
      });
      ...
      this.publish({
        ...
        pendingWrite: false,
        notice:
          "Your meal was replaced, but the week could not refresh. Reload the week; do not replace it again.",
      });
      ...
      this.publish({
        ...
        pendingWrite: false,
        notice:
          "The week changed or this meal could not be replaced. Reload and check the slot before trying again.",
      });

...
export type PreparationView = {
  ...
  notice: string | null;
  receipt: MealPreparationReceipt | null;
};
export class MealPreparationRuntime {
  private view: PreparationView = {
    snapshot: null,
    members: [],
    busy: false,
    pendingWrite: false,
  ...
  private attempt: CreateMealPreparation | null = null;
  private confirmed: MealPreparationReceipt | null = null;
  ...
  private publish(patch: Partial<PreparationView>) {
  ...
  private async read() {
    ...
    this.publish({
      ...
      notice: this.confirmed
        ? "Preparation was created. These details show the current household task."
        : null,
    });
  }
  ...
  save = async (preparation: unknown) => {
    if (!this.canSave()) return;
    const snapshot = this.view.snapshot!;
    const parsed = Schema.decodeUnknownExit(CreateMealPreparation)(
      {
        ...this.target,
    ...
    const assignment = parsed.value.preparation.assignment;
    const member =
    ...
    this.attempt = {
      ...parsed.value,
      preparation: { ...parsed.value.preparation, assignment: { ...assignment } },
    };
    await this.send();
  };
  private canSave() {
  ...
  private async send() {
    ...
    try {
      const receipt = await this.run(this.client.meals.createPreparation(this.attempt));
      if (this.disposed) return;
  ...
  private failed(error: unknown) {
    ...
    if (code === "session" || code === "forbidden") {
      ...
      this.publish({
        ...
        pendingWrite: false,
        notice:
          "Preparation was created, but its details could not refresh. Reload without creating it again.",
      });
  ...
  private unavailable() {
    this.publish({
      ...
      notice: this.attempt
        ? "Creation is unconfirmed. Retry this exact request before another change."

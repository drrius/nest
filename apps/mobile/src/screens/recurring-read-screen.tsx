    ...
    <MoneyScreenGate>
      {(props) => (
        <OwnedRead
          {...props}
          target={target}
          key={`${props.account.session.lease}:${target.kind === "list" || target.kind === "legacy" || target.kind === "due-variable" ? target.kind : `${target.kind}:${"draftId" in target ? target.draftId : target.ruleId}`}`}
        />
      )}
    </MoneyScreenGate>
  );

...
export function ReminderRecovery({
  ...
  if (!view.active) return <Note>Meal details are hidden while inactive.</Note>;
  return <RecoveryContent runtime={runtime} entryId={entryId} context={context} next={next} />;
}
function RecoveryContent({
  ...
  const review = recoveryReview(runtime, entryId, context);
...
function Terminal({
  ...
  return (
    <>
      <Note>
        {recorded
          ? "Reminder settings saved. The meal is unchanged."
...
function recoveryReview(
  ...
  return command ? reminderRecoveryReview(command, entryId, context) : null;
}
function OtherTarget({ target, disabled }: { target: string; disabled: boolean }) {
  ...
  return (
    <>
      <Note>
        An earlier reminder change belongs to another meal. Open that meal to review its settings
        and outcome.
      </Note>
      <NativeAction
        label="Open pending reminder"
        disabled={disabled}
        onPress={() => router.replace({ pathname: "/meal-reminder", params: { entryId: target } })}
      />
    </>
  );
}

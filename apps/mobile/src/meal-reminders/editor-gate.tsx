...
export function ReminderEditorGate({
  ...
  if (session.state.status !== "ready" || !session.mealReminders || !session.routines)
    return (
      <Page>
        <Note>Sign in to manage meal reminders.</Note>
      </Page>
    );
  ...
  return children({
    ...
    reminders: session.mealReminders,

...
export function ReminderEditorBody(
  props: ReminderEditorAccount & { entryId: string; runtime: MealReminderSaveRuntime },
) {
  const { runtime } = props;
  ...
  const context = useReminderEditorContext(props.account, clients, props.entryId, view);
  ...
  return (
    <Page>
      {!view.online ? <Note>Connect to manage meals. Changes are not queued offline.</Note> : null}
      {view.notice ? <Note>{view.notice}</Note> : null}
      {context.failed ? <Note>Could not load meal choices. Reload to retry.</Note> : null}
      {view.attempt || view.result ? (
        ...
        <ReminderEditorForm runtime={runtime} context={context} entryId={props.entryId} />
      )}
      <NativeAction
        label="Reload meal and request status"

...
export function ReminderRecovery({
  ...
  if (!view.active) return <Note>Recurring details are hidden while inactive.</Note>;
  return <RecoveryContent runtime={runtime} ruleId={ruleId} context={context} next={next} />;
}
function RecoveryContent({
  ...
  const review = recoveryReview(runtime, ruleId, context);
...
function Terminal({
  ...
  return (
    <>
      <Note>
        {recorded
          ? "Reminder settings saved. The recurring expense is unchanged."
...
function recoveryReview(
  ...
  return command ? reminderRecoveryReview(command, ruleId, context) : null;
}
function OtherTarget({ target, disabled }: { target: string; disabled: boolean }) {
  ...
  return (
    <>
      <Note>
        An earlier reminder change belongs to another recurring. Open that recurring to review its
      ...
      <NativeAction
        ...
        disabled={disabled}
        onPress={() =>
          router.replace({ pathname: "/recurring-reminder", params: { ruleId: target } })
        }
      />
    </>
  );

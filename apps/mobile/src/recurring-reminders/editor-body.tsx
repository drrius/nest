...
export function ReminderEditorBody(
  props: ReminderEditorAccount & { ruleId: string; runtime: RecurringReminderSaveRuntime },
) {
  const { runtime } = props;
  ...
  const context = useReminderEditorContext(props.account, clients, props.ruleId, view);
  ...
  return (
    <Page>
      {!view.online ? (
        <Note>Connect to manage recurring reminders. Changes are not queued offline.</Note>
      ) : null}
      {view.notice ? <Note>{view.notice}</Note> : null}
      {context.failed ? <Note>Could not load the recurring expense. Reload to retry.</Note> : null}
      {view.attempt || view.result ? (
        ...
        <ReminderEditorForm runtime={runtime} context={context} ruleId={props.ruleId} />
      )}
      <NativeAction
        label="Reload recurring expense and request status"

import { matchesAdoptionContext } from "./legacy-adoption-approval-display.ts";
...
import type { RecurringStateApprovalAttempt as LegacyAdoptionApprovalAttempt } from "./recurring-state-approval-attempt.ts";
import type {
  LegacyAdoptionApprovalOperations,
  LegacyAdoptionApprovalContext,
} from "./legacy-adoption-approval-operations.ts";
export interface LegacyAdoptionApprovalView {
  approval: LegacyAdoptionApproval | null;
  context: LegacyAdoptionApprovalContext | null;
  attempt: LegacyAdoptionApprovalAttempt | null;
  active: boolean;
  online: boolean;
  busy: boolean;
  fresh: boolean;
...
const terminal = (approval: LegacyAdoptionApproval) =>
  ["consumed", "denied"].includes(approval.status);
export class LegacyAdoptionApprovalRuntime {
  private view: LegacyAdoptionApprovalView = {
    approval: null,
    context: null,
    attempt: null,
    active: false,
  ...
  private readonly operations: LegacyAdoptionApprovalOperations;
  private readonly approvalId: string;
  private readonly now: () => number;
  private readonly listeners = new Set<() => void>();
  private request: AbortController | null = null;
  private disposed = false;
  constructor(operations: LegacyAdoptionApprovalOperations, approvalId: string, now = Date.now) {
  ...
  private publish(patch: Partial<LegacyAdoptionApprovalView>) {
  ...
  decide = async (expected: LegacyAdoptionApproval, approved: boolean) => {
    if (
      !this.available() ||
      !this.view.fresh ||
      this.view.attempt ||
      ...
      terminal(expected) ||
      (approved && !matchesAdoptionContext(expected, this.view.context))
    )
      return;
  ...
  withdraw = async (expected: LegacyAdoptionApproval) => {
  ...
  private async send(
    ...
    attempt: LegacyAdoptionApprovalAttempt,
  ...
  private async clear(attempt: LegacyAdoptionApprovalAttempt | null, request: AbortController) {
    if (!attempt) return;

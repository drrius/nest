import {
  matchesVariableCycleContext,
  variableCycleSuperseded,
} from "./recurring-variable-approval-display.ts";
...
import type { RecurringStateApprovalAttempt as VariableCycleApprovalAttempt } from "./recurring-state-approval-attempt.ts";
import type {
  VariableCycleApprovalOperations,
  VariableCycleApprovalContext,
} from "./recurring-variable-approval-operations.ts";
export interface VariableCycleApprovalView {
  approval: VariableCycleApproval | null;
  context: VariableCycleApprovalContext | null;
  attempt: VariableCycleApprovalAttempt | null;
  active: boolean;
  online: boolean;
  busy: boolean;
  fresh: boolean;
...
const terminal = (approval: VariableCycleApproval) =>
  ["consumed", "denied"].includes(approval.status);
export class VariableCycleApprovalRuntime {
  private view: VariableCycleApprovalView = {
    approval: null,
    context: null,
    attempt: null,
    active: false,
  ...
  private readonly operations: VariableCycleApprovalOperations;
  private readonly approvalId: string;
  private readonly now: () => number;
  private readonly listeners = new Set<() => void>();
  private request: AbortController | null = null;
  private disposed = false;
  constructor(operations: VariableCycleApprovalOperations, approvalId: string, now = Date.now) {
  ...
  private publish(patch: Partial<VariableCycleApprovalView>) {
  ...
  refresh = async () => {
    ...
    await this.perform(async (request) => {
      ...
      const context = terminal(approval)
        ? null
        : await Effect.runPromise(this.operations.context(approval.input), {
            signal: request.signal,
          });
      if (!this.current(request)) return;
  ...
  private async recover(
    loaded: Pick<VariableCycleApprovalView, "approval" | "context" | "attempt"> & {
    ...
    const superseded = Boolean(attempt?.approved && variableCycleSuperseded(approval, context));
  ...
  decide = async (expected: VariableCycleApproval, approved: boolean) => {
    if (
      !this.available() ||
      !this.view.fresh ||
      this.view.attempt ||
      ...
      terminal(expected) ||
      (approved && !matchesVariableCycleContext(expected, this.view.context))
    )
      return;
  ...
  private async send(
    approval: VariableCycleApproval,
    attempt: VariableCycleApprovalAttempt,
    request: AbortController,
  ) {
  ...
  private async clear(attempt: VariableCycleApprovalAttempt | null, request: AbortController) {
    if (!attempt) return;
...
function recoveryNotice(
  ...
  if (superseded)
    return `The rule or cycle changed. This proposal can no longer be approved. You can decline it and request a new proposal.`;
  return attempt

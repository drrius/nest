...
import { matchesCorrectionContext } from "./correction-approval-display.ts";
...
export interface CorrectionApprovalView {
  approval: CorrectionApproval | null;
  context: CorrectionContext | null;
  attempt: CorrectionApprovalAttempt | null;
  active: boolean;
  online: boolean;
  busy: boolean;
  fresh: boolean;
...
const terminal = (approval: CorrectionApproval) => ["consumed", "denied"].includes(approval.status);
export class CorrectionApprovalRuntime {
  private view: CorrectionApprovalView = {
    approval: null,
    context: null,
    attempt: null,
    active: false,
  ...
  private readonly operations: CorrectionApprovalOperations;
  private readonly approvalId: string;
  private readonly now: () => number;
  private readonly listeners = new Set<() => void>();
  private request: AbortController | null = null;
  private disposed = false;
  constructor(operations: CorrectionApprovalOperations, approvalId: string, now = Date.now) {
  ...
  private publish(patch: Partial<CorrectionApprovalView>) {
  ...
  refresh = async () => {
    ...
    await this.perform(async (request) => {
      ...
      const context = terminal(approval)
        ? null
        : await Effect.runPromise(this.operations.context(approval.correction.sourceEventId), {
            signal: request.signal,
          });
      if (!this.current(request)) return;
      this.publish({
  ...
  decide = async (expected: CorrectionApproval, approved: boolean) => {
    if (
      !this.available() ||
      !this.view.fresh ||
      this.view.attempt ||
      ...
      terminal(expected) ||
      (approved && !matchesCorrectionContext(expected, this.view.context))
    )
      return;
  ...
  private async send(
    ...
    const result = await Effect.runPromise(
      this.operations.decide({ ...attempt, correction: approval.correction }),
      { signal: request.signal },
    );
    if (!this.current(request)) return;
  ...
  private async clear(attempt: CorrectionApprovalAttempt | null, request: AbortController) {
    if (!attempt) return;

import {
  matchesManualCycleContext,
  manualCycleSuperseded,
} from "./recurring-manual-approval-display.ts";
...
import type { RecurringStateApprovalAttempt as ManualCycleApprovalAttempt } from "./recurring-state-approval-attempt.ts";
import type {
  ManualCycleApprovalOperations,
  ManualCycleApprovalContext,
} from "./recurring-manual-approval-operations.ts";
export interface ManualCycleApprovalView {
  approval: ManualCycleApproval | null;
  context: ManualCycleApprovalContext | null;
  attempt: ManualCycleApprovalAttempt | null;
  active: boolean;
  online: boolean;
  busy: boolean;
  fresh: boolean;
...
const terminal = (approval: ManualCycleApproval) =>
  ["consumed", "denied"].includes(approval.status);
export class ManualCycleApprovalRuntime {
  private view: ManualCycleApprovalView = {
    approval: null,
    context: null,
    attempt: null,
    active: false,
  ...
  private readonly operations: ManualCycleApprovalOperations;
  private readonly approvalId: string;
  private readonly now: () => number;
  private readonly listeners = new Set<() => void>();
  private request: AbortController | null = null;
  private disposed = false;
  constructor(operations: ManualCycleApprovalOperations, approvalId: string, now = Date.now) {
  ...
  private publish(patch: Partial<ManualCycleApprovalView>) {
  ...
  private async recover(
    loaded: Pick<ManualCycleApprovalView, "approval" | "context" | "attempt"> & {
    ...
    const superseded = Boolean(attempt?.approved && manualCycleSuperseded(approval, context));
  ...
  decide = async (expected: ManualCycleApproval, approved: boolean) => {
    if (
      !this.available() ||
      !this.view.fresh ||
      this.view.attempt ||
      ...
      terminal(expected) ||
      (approved && !matchesManualCycleContext(expected, this.view.context))
    )
      return;
  ...
  private async send(
    approval: ManualCycleApproval,
    attempt: ManualCycleApprovalAttempt,
    request: AbortController,
  ) {
  ...
  private async clear(attempt: ManualCycleApprovalAttempt | null, request: AbortController) {
    if (!attempt) return;

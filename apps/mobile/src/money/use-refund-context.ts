...
import { refundEntryOptions } from "./refund-entry-options";
import type { MoneyScreenAccount } from "./screen-gate";
interface Loaded {
  token: object;
  value: typeof RefundContext.Type | null;
  verify: boolean;
  failed: boolean;
}
export function useRefundContext(
  ...
  useEffect(() => {
    ...
    void Effect.runPromise(refundEntryOptions(props.account, props.client, props.sourceEventId), {

...
export interface RefundApprovalView {
  ...
  attempt: RefundApprovalAttempt | null;
  active: boolean;
  online: boolean;
  busy: boolean;
  fresh: boolean;
...
const terminal = (approval: RefundApproval) => ["consumed", "denied"].includes(approval.status);
export class RefundApprovalRuntime {
  private view: RefundApprovalView = {
    approval: null,
    attempt: null,
    active: false,
    online: false,
  ...
  private readonly operations: RefundApprovalOperations;
  private readonly approvalId: string;
  private readonly now: () => number;
  private readonly listeners = new Set<() => void>();
  private request: AbortController | null = null;
  private disposed = false;
  constructor(operations: RefundApprovalOperations, approvalId: string, now = Date.now) {
  ...
  private publish(patch: Partial<RefundApprovalView>) {
  ...
  decide = async (expected: RefundApproval, approved: boolean) => {
    if (
      !this.available() ||
      !this.view.fresh ||
      this.view.attempt ||
  ...
  private async send(
    ...
    const result = await Effect.runPromise(
      this.operations.decide({ ...attempt, refund: approval.refund }),
      { signal: request.signal },
    );
    if (!this.current(request)) return;
  ...
  private async clear(attempt: RefundApprovalAttempt | null, request: AbortController) {
    if (!attempt) return;

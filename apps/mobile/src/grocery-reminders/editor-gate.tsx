...
export function ReminderEditorGate({
  ...
  if (session.state.status !== "ready" || !session.groceryReminders || !session.routines)
    return (
      <Page>
        <Note>Sign in to manage grocery reminders.</Note>
      </Page>
    );
  ...
  return children({
    ...
    reminders: session.groceryReminders,

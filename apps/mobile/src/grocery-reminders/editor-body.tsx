...
export function ReminderEditorBody(
  props: ReminderEditorAccount & { itemId: string; runtime: GroceryReminderSaveRuntime },
) {
  const { runtime } = props;
  ...
  const context = useReminderEditorContext(props.account, clients, props.itemId, view);
  ...
  return (
    <Page>
      {!view.online ? (
        <Note>Connect to manage groceries. Changes are not queued offline.</Note>
      ) : null}
      {view.notice ? <Note>{view.notice}</Note> : null}
      {context.failed ? <Note>Could not load grocery details. Reload to retry.</Note> : null}
      {view.attempt || view.result ? (
        ...
        <ReminderEditorForm runtime={runtime} context={context} itemId={props.itemId} />
      )}
      <NativeAction
        label="Reload grocery item and request status"

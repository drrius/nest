...
function Form(props: Props & { initial: ReminderEditorContext }) {
  ...
  const submit = () => {
    ...
    if (!settings)
      return setError("Choose recipients and enter a valid YYYY-MM-DD date and HH:MM time.");
    ...
    const dialog = reminderConfirmation(
      {
        operationId,
        itemId: initial.grocery.itemId,
        expectedItemVersion: initial.itemVersion,
...
function matching(context: Props["context"], initial: ReminderEditorContext) {
  ...
  return (
    value.itemVersion === initial.itemVersion &&

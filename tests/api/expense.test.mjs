...
import { expenseCommands } from "../../apps/api/src/money/expense.ts";
...
const run = (value, input = command, action = "save") =>
  Effect.runPromise(
    expenseCommands(config, caller)
      [action](input)
      .pipe(Effect.provideService(FetchHttpClient.Fetch, async () => Response.json(value))),
  );
test("expense service binds full receipt and rejects hidden authorization fields", async () => {
    ...
    expenseCommands(config, caller)
      ...
      .pipe(
        Effect.provideService(FetchHttpClient.Fetch, async (url, init) => {
          assert.equal(new URL(url).pathname, "/rest/v1/rpc/nest_execute_expense");

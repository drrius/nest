...
import { createMealPreparation } from "../../apps/api/src/meals/preparation-create.ts";
...
const receipt = {
  ...
  dueOn: input.preparation.dueOn,
};
const run = (value, fetch) =>
  Effect.runPromise(
    createMealPreparation(config, caller, value).pipe(
      Effect.provideService(FetchHttpClient.Fetch, fetch),
    ),
  );

test("preparation creation binds actor, household, exact bigint revision and canonical retry identity", async () => {
  let calls = 0;
  assert.deepEqual(
    await run(input, async (url, init) => {
      calls++;
      assert.equal(new URL(url).pathname, "/rest/v1/rpc/nest_create_meal_preparation");

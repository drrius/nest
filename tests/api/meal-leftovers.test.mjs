...
import { placeLeftovers } from "../../apps/api/src/meals/leftovers.ts";
...
const receipt = {
  ...
  entryId: id(200),
  sourceEntryId: input.entryId.toLowerCase(),
...
const run = (value, fetch) =>
  Effect.runPromise(
    placeLeftovers(config, caller, value).pipe(Effect.provideService(FetchHttpClient.Fetch, fetch)),
  );

test("leftover placement binds actor, household, exact bigint revision and canonical retry identity", async () => {
  let calls = 0;
  assert.deepEqual(
    await run(input, async (url, init) => {
      calls++;
      assert.equal(new URL(url).pathname, "/rest/v1/rpc/nest_place_leftovers");
...
test("invalid and injected leftover inputs fail before upstream dispatch", async () => {
  let calls = 0;
  for (const patch of [
    { actorId: id(2) },
    { householdId: id(20) },
...
test("same-week leftovers advance the common baseline once while cross-week zero source revision remains valid", async () => {
  const same = {
    ...input,
  ...
  const saved = {
    ...receipt,
    targetWeekStart: same.targetWeekStart,
    date: same.date,
    sourceRevision: "9007199254740994",
    targetRevision: "9007199254740994",
  };
  assert.deepEqual(await run(same, async () => Response.json(saved)), saved);
  const legacy = { ...input, expectedSourceRevision: "0" };
  assert.deepEqual(
    await run(legacy, async () => Response.json({ ...receipt, sourceRevision: "0" })),

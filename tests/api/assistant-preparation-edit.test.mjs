...
test("AI preparation binds exact source, unchanged week revision, due date and caller", async () => {
  const calls = [];
  assert.deepEqual(await run("editMealPreparation", command, fetcher(receipt, calls)), receipt);
  assert.deepEqual(calls[0].p_input, command);
  assert.equal(calls[0].p_tool, "editMealPreparation");
  for (const patch of [
    { actorId: id(2) },
    { householdId: id(20) },
    { entryId: id(99) },
    ...
    await assert.rejects(run("editMealPreparation", command, fetcher({ ...receipt, ...patch })), {
      code: "unavailable",
    });
  for (const patch of [
    { actorId: id(2) },
    { operationId: id(9) },
    { expectedRevision: "-1" },
    { patch: { ...command.patch, householdId: id(20) } },
  ])
    await assert.rejects(
      run("editMealPreparation", { ...command, ...patch }, fetcher(receipt, calls)),
      { code: "unavailable" },
    );

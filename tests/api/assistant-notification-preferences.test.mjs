...
const run = (fetch) =>
  Effect.runPromise(
    execute("saveNotificationPreferences", command, "notification-call").pipe(
      Effect.provideService(FetchHttpClient.Fetch, fetch),
    ),
  );
test("notification journal adapter verifies saved actor, household and exact resulting revision", async () => {
  const calls = [];
  ...
  assert.deepEqual(calls[0], {
    ...
    p_call: "notification-call",
    p_tool: "saveNotificationPreferences",
    p_input: command,
  });
  for (const patch of [
    { actorId: id(2) },
...
test("notification journal adapter cannot follow membership into a different household", async () => {
  const calls = [];

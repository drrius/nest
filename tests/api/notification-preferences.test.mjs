...
import { notificationPreferences } from "../../apps/api/src/notifications/service.ts";
...
const service = notificationPreferences(
...
test("invalid profile command fields fail before any mutation request", async () => {
  for (const input of [
    ...
    { ...command, preferences: { ...preferences, dailySummaryEnabled: "true" } },
    { ...command, preferences: { ...preferences, dailySummaryTime: "24:00" } },
    { ...command, preferences: { ...preferences, itemRemindersEnabled: null } },
  ]) {
    let calls = 0;
    await assert.rejects(

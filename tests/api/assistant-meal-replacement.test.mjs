...
test("AI meal receipt binds actor, household, week, entry and exact bigint increment", async () => {
  const calls = [];
  assert.deepEqual(await run("replaceMeal", command, fetcher(receipt, calls)), receipt);
  ...
  for (const patch of [
    ...
    await assert.rejects(run("replaceMeal", command, fetcher({ ...receipt, ...patch })), {
      code: "unavailable",
    });
});
test("AI meal model cannot supply retry or account identities or bypass the shared date constraint", async () => {
  ...
  for (const patch of [
    ...
    await assert.rejects(run("replaceMeal", { ...command, ...patch }, fetcher(receipt, calls)), {
      code: "unavailable",
    });

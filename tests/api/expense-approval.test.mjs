...
import { expenseApprovals } from "../../apps/api/src/money/expense-approval.ts";
...
const input = { approvalId: id(100), operationId: id(101), expense: payload(), approved: true };
const run = (response, action = "read", value = { approvalId: id(100) }) =>
  Effect.runPromise(
    expenseApprovals(config, caller)
      [action](value)
      .pipe(Effect.provideService(FetchHttpClient.Fetch, async () => Response.json(response))),
  );
test("approval response binds owner, exact expense and receipt; a missing committed receipt is never shown as success", async () => {
  ...
  for (const response of [
    ...
    },
    {
      ...
      approval: {
        ...consumed.approval,
        receipt: { ...receipt, expense: payload({ note: "Changed" }) },
      },
    },
  ])
  ...
  await assert.rejects(
    run(consumed, "decide", { ...input, expense: payload({ note: "Changed" }) }),
    { code: "unavailable" },
  );

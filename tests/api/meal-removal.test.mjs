...
import { removeMeal } from "../../apps/api/src/meals/removal.ts";
...
const run = (value, fetch) =>
  Effect.runPromise(
    removeMeal(config, caller, value).pipe(Effect.provideService(FetchHttpClient.Fetch, fetch)),
  );

test("removal binds actor, household, exact bigint revision and canonical retry identity", async () => {
  let calls = 0;
  assert.deepEqual(
    await run(input, async (url, init) => {
      calls++;
      assert.equal(new URL(url).pathname, "/rest/v1/rpc/nest_remove_meal");
...
test("invalid and injected removal inputs fail before upstream dispatch", async () => {
  let calls = 0;
  for (const patch of [
    { actorId: id(2) },
    { householdId: id(20) },

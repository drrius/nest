...
test("notification device handoff reauthorizes and makes no preference or enrollment writes", async (t) => {
  ...
  const tool = setupTools(request, config).openNotificationSetup;
  const options = { toolCallId: "handoff", messages: [] };
  assert.deepEqual(await tool.execute({}, options), {
    ok: true,
    value: { kind: "device_handoff", screen: "notification-preferences" },
  });
  revoked = true;

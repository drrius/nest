...
import { refundCommands } from "../../apps/api/src/money/refund.ts";
import { id } from "../database/native-expense-helpers.mjs";
import { refund } from "../integration/refund-api-fixture.mjs";
...
const run = (value, input = command, action = "save") =>
  Effect.runPromise(
    refundCommands(config, caller)
      [action](input)
      .pipe(Effect.provideService(FetchHttpClient.Fetch, async () => Response.json(value))),
  );
test("refund service binds full receipt and rejects hidden authorization fields", async () => {
    ...
    refundCommands(config, caller)
      ...
      .pipe(
        Effect.provideService(FetchHttpClient.Fetch, async (url, init) => {
          assert.equal(new URL(url).pathname, "/rest/v1/rpc/nest_execute_refund");

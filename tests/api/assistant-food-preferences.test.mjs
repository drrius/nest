...
const run = (fetch) =>
  Effect.runPromise(
    execute("saveFoodPreferences", command, "food-call").pipe(
      Effect.provideService(FetchHttpClient.Fetch, fetch),
    ),
  );
test("food journal adapter verifies saved actor, household and exact resulting revision", async () => {
  const calls = [];
...
test("food journal adapter cannot follow membership into a different household", async () => {
  const calls = [];

...
import { settlementApprovals } from "../../apps/api/src/money/settlement-approval.ts";
...
const input = { approvalId: id(100), operationId: id(101), settlement: payload(), approved: true };
const run = (response, action = "read", value = { approvalId: id(100) }) =>
  Effect.runPromise(
    settlementApprovals(config, caller)
      [action](value)
      .pipe(Effect.provideService(FetchHttpClient.Fetch, async () => Response.json(response))),
  );
test("approval response binds owner, exact settlement and receipt; a missing committed receipt is never shown as success", async () => {
  ...
  for (const response of [
    ...
    },
    {
      ...
      approval: {
        ...consumed.approval,
        receipt: { ...receipt, settlement: payload({ note: "Changed" }) },
      },
    },
  ])
  ...
  await assert.rejects(
    run(consumed, "decide", { ...input, settlement: payload({ note: "Changed" }) }),
    { code: "unavailable" },
  );

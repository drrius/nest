...
test("AI edit receipt binds canonical recipe identity and exact bigint revision", async () => {
  const calls = [],
    value = { ...command, definitionId: command.definitionId.toUpperCase() };
  assert.deepEqual(await run("editRecipe", value, fetcher(receipt, calls)), receipt);
  assert.equal(calls[0].p_tool, "editRecipe");
  ...
  for (const patch of [
    ...
    await assert.rejects(run("editRecipe", command, fetcher({ ...receipt, ...patch })), {
      code: "unavailable",
    });
});
test("AI edit rejects injected identities and invalid revisions before dispatch", async () => {
  const calls = [];
  for (const patch of [
    { actorId: id(2) },
    { householdId: id(20) },
    ...
    await assert.rejects(run("editRecipe", { ...command, ...patch }, fetcher(receipt, calls)), {
      code: "unavailable",
    });
  assert.equal(calls.length, 0);
});

test("AI edit uses UTF-8 size bounds before journal dispatch", async () => {
  const calls = [],
  ...
  assert.deepEqual(await run("editRecipe", value, fetcher(result, calls)), result);
  assert.equal(calls.length, 1);
  await assert.rejects(run("editRecipe", boundedEditInput(49153), fetcher(result, calls)), {

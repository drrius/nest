...
import { editRoutine } from "../../apps/api/src/routines/edit.ts";
...
test("edit sends only the requested patch and exact microsecond baseline with verified scope", async () => {
  assert.deepEqual(
    await run(command, (url, init) => {
      assert.equal(new URL(url).pathname, "/rest/v1/rpc/nest_edit_routine");
...
test("edit rejects mismatched receipt identity, action, routine and unknown fields", async () => {
  for (const change of [
    { actorId: id(2) },
    { householdId: id(20) },
    { operationId: id(201) },
...
test("invalid edits cannot reach the database transport", async () => {
  for (const change of [
    { actorId: id(2) },
    { householdId: id(20) },
    { patch: {} },

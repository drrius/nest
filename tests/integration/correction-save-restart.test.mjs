...
for (const action of ["save", "cancel"]) {
  test(`direct ${action} recovers real response loss across SQLite restart with zero automatic sends`, async (t) => {
    ...
    const account = { actor: id(1), household: id(10) },
      command = { operationId: id(100), correction: payload(f.source) };
    const session = await run(local.store.activate(account, id(900)));
    const proxy = await lostResponseProxy(t, f.url, `/v1/money/correction/${action}`);
    ...
    const client = {
      recoverCorrection: (input) =>
        raw.recoverCorrection(input).pipe(Effect.provideService(Fetch.Fetch, fetch)),
      saveCorrection: (input) => {
        sends++;
        return raw.saveCorrection(input).pipe(Effect.provideService(Fetch.Fetch, fetch));
      },
      cancelCorrection: (input) => {
        sends++;
        return raw.cancelCorrection(input).pipe(Effect.provideService(Fetch.Fetch, fetch));
      },
    };
    if (action === "cancel")
      await run(local.store.stageCorrectionSave(session, { action: "save", command }, () => true));
    const runtime = new CorrectionSaveRuntime(
      correctionSaveOperations({ store: local.store, session }, client),
    );
    ...
    assert.deepEqual(await run(local.store.readCorrectionSave(session)), { action, command });
    runtime.dispose();
    const reopened = local.reopen();
    const recovered = new CorrectionSaveRuntime(
      correctionSaveOperations({ store: reopened.store, session }, client),
    );
    ...
    assert.equal(sends, 1);
    assert.equal(await run(reopened.store.readCorrectionSave(session)), null);
    assert.equal(

...
import { fixture as server, id, run } from "./recurring-reminder-fixture.mjs";
import { fixture as sqlite } from "../../apps/mobile/tests/offline-fixture.mjs";
import { recurringReminderSaveOperations } from "../../apps/mobile/src/recurring-reminders/save-operations.ts";
import { RecurringReminderSaveRuntime } from "../../apps/mobile/src/recurring-reminders/save-runtime.ts";
export { id, run };
...
export async function fixture(t) {
  ...
  const transport = async (input, init) => {
    const save = String(input).endsWith("/recurring-reminders/save");
    if (save) {
      sends++;
  ...
  const operations = (store = local.store) =>
    recurringReminderSaveOperations({ store, session }, client);
  const mount = async (store = local.store) => {
    const runtime = new RecurringReminderSaveRuntime(operations(store));

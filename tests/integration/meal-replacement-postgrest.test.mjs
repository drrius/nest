...
async function backend(t, loseResponse = false) {
  ...
  const proxy = loseResponse
    ? await lostResponseProxy(t, remote.url, "/rest/v1/rpc/nest_replace_meal")
    : null;
  ...
  const connect = (bearer = remote.bearer) => {
    ...
    return {
      ...
      replace: (input = command) =>
        fetch(`${url}/v1/meals/replace`, { method: "POST", headers, body: JSON.stringify(input) }),
    };
  };
  return { remote, proxy, url, connect };
}

test("replacement HTTP retries its original receipt after a partner edits the replacement and rejects revoked recovery", async (t) => {
  const { remote, proxy, url, connect } = await backend(t, true);
  const owner = connect();
  assert.equal((await owner.replace()).status, 503);
  assert.equal(proxy.dropped(), 1);
  const stored = JSON.parse(
    remote.db.sql("select result from public.nest_meal_replacement_receipts"),
  );
  remote.db.sql(
    `update public.meal_plan_entries set title_snapshot='Partner edit' where id='${stored.entryId}'`,
  );
  const retry = await owner.replace();
  ...
  assert.equal(
    remote.db.sql(
      `select title_snapshot from public.meal_plan_entries where id='${stored.entryId}'`,
    ),
    "Partner edit",
  );
  assert.equal(
    remote.db.sql(
      `select removed_at is not null from public.meal_plan_entries where id='${id(100)}'`,
    ),
    "t",
  );
  assert.equal(
    (await connect(remote.partnerBearer).replace({ ...command, operationId: id(202) })).status,
    409,
  );
  assert.equal((await connect(remote.otherBearer).replace()).status, 403);
  assert.equal((await fetch(`${url}/v1/meals/replace`, { method: "POST" })).status, 401);
  assert.equal((await fetch(`${url}/v1/meals/replace`)).status, 405);
  assert.equal((await owner.replace({ ...command, actorId: id(2) })).status, 400);
  remote.db.sql(`delete from public.household_members where user_id='${id(1)}'`);
  assert.equal((await owner.replace()).status, 403);
});

test("replacement HTTP closes actual linked preparation once and partner reads retain the removed history", async (t) => {
  ...
  const response = await connect().replace();
  ...
  assert.deepEqual(await (await connect().replace()).json(), result);
  const week = await (await connect(remote.partnerBearer).read()).json();
  assert.equal(
    week.entries.find((entry) => entry.entryId === result.receipt.entryId).title,
    command.title,
  );
  assert.equal(
...
test("uppercase entry identity succeeds and replays through real HTTP and PostgreSQL", async (t) => {
  ...
  const input = { ...command, entryId: entry, expectedRevision: "2", date: "2026-09-23" };
  const first = await connect().replace(input);
  assert.equal(first.status, 200);
  const result = await first.json();
  assert.equal(result.receipt.previousEntryId, entry.toLowerCase());
  const retry = await connect().replace(input);
  assert.equal(retry.status, 200);
  assert.deepEqual(await retry.json(), result);
  assert.equal(remote.db.sql("select count(*) from public.nest_meal_replacement_receipts"), "1");
});

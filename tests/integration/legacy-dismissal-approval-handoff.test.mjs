...
import { files, id, json, payload } from "../database/ai-legacy-dismissal-fixture.mjs";
...
import { legacyDismissalApprovalOwner } from "../../apps/mobile/src/money/legacy-dismissal-approval-owner.ts";
import { legacyDismissalApprovalActions } from "../../apps/mobile/src/money/legacy-dismissal-approval-display.ts";
...
async function proposal(f, config) {
  const turn = {
    ...
    operationId: id(901),
    expectedRevision: "0",
    text: "Dismiss the explicitly selected retained draft",
  };
  const message = {
    id: turn.operationId,
    role: "user",
  ...
  const result = await tools.proposeLegacyDismissal.execute(input, {
    toolCallId: "recurring",
    messages: [],
  });
  assert.deepEqual(
    await tools.proposeLegacyDismissal.execute(input, { toolCallId: "recurring", messages: [] }),
    result,
  );
  return result;
}
test("real AI tool returns a private native handoff; only exact confirmation dismisses the retained draft", async (t) => {
  ...
  assert.equal(card.href.pathname, "/legacy-dismissal-approval");
  assert.match(card.label, /not dismissed yet/);
  ...
  const client = Object.fromEntries(
    ["legacyDismissalApproval", "legacyDismissalContext", "decideLegacyDismissal"].map((name) => [
      name,
      (...args) => raw[name](...args).pipe(Effect.provideService(Fetch.Fetch, fetch)),
    ]),
  );
  const owner = legacyDismissalApprovalOwner(
    legacyDismissalApprovalOperations({ store: local.store, session }, client),
  ...
  assert.equal(legacyDismissalApprovalActions(view, Date.now()).confirm, true);

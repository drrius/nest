...
import { correctionSaveTool } from "../../apps/api/src/money/correction-save-tool.ts";
...
const commandFor = (f) => ({ operationId: id(100), correction: correction(f.source) });
test("native and SDK Save recovery read a lost committed receipt without reposting or leaking to a partner", async (t) => {
  const f = await correctionApiFixture(t),
    proxy = await lostResponseProxy(t, f.url, "/v1/money/correction/save");
  const command = commandFor(f);
  const native = client(f, id(1), f.bearer, proxy.url);
  assert.equal((await run(native.recoverCorrection(command))).status, "unresolved");
  await assert.rejects(run(native.saveCorrection(command)), { code: "unavailable" });
  const { receipt } = await run(client(f).recoverCorrection(command));
  assert.equal(receipt.operationId, command.operationId);
  assert.equal(receipt.approvalId, null);
  assert.equal(
    (await run(client(f, id(2), f.partnerBearer).recoverCorrection(command))).status,
    "unresolved",
  );
  const request = new Request(f.url, {
    headers: { authorization: `Bearer ${f.bearer}`, "x-nest-household": id(10) },
  });
  const tool = correctionSaveTool(request, {
    url: f.supabaseUrl,
  ...
  assert.equal(
    (
      await fetch(`${f.url}/v1/money/correction/receipt?operationId=${id(100)}&actorId=${id(2)}`, {
        headers,
      })
    ).status,
    400,
  );
  const readUrl = `${f.url}/v1/money/correction/receipt?operationId=${id(100)}`;
  ...
  const denied = await fetch(
    `${f.supabaseUrl}/rest/v1/nest_correction_receipts?select=result&operation_id=eq.${id(100)}`,
...
test("missing receipt during a blocked Save is not a terminal failure; later recovery sees the single commit", async (t) => {
  ...
  const saving = run(native.saveCorrection(command));
  await waitFor(f.db, "query like '%nest_save_correction%' and wait_event='advisory'");
  assert.equal((await run(native.recoverCorrection(command))).status, "unresolved");
  await holder;
  const receipt = await saving;
  assert.deepEqual((await run(native.recoverCorrection(command))).receipt, receipt);
  assert.equal(f.db.sql("select count(*) from public.financial_events"), "2");
});
test("revoked membership cannot recover even a missing Save receipt", async (t) => {
  const f = await correctionApiFixture(t, false),
    native = client(f),
    command = commandFor(f);
  f.db.sql(`delete from public.household_members where user_id='${id(1)}'`);
  await assert.rejects(run(native.recoverCorrection(command)), { code: "forbidden" });
});

test("lost cancellation acknowledgement recovers a permanent outcome and blocks a late Save", async (t) => {
  const f = await correctionApiFixture(t),
    proxy = await lostResponseProxy(t, f.url, "/v1/money/correction/cancel");
  const command = commandFor(f);
  await assert.rejects(run(client(f, id(1), f.bearer, proxy.url).cancelCorrection(command)), {
    code: "unavailable",
  });
  const native = client(f);
  assert.equal((await run(native.recoverCorrection(command))).status, "cancelled");
  await assert.rejects(run(native.saveCorrection(command)), { code: "conflict" });
  assert.equal((await run(native.cancelCorrection(command))).status, "cancelled");
...
test("cancelling after a lost Save returns the recorded receipt without reversing history", async (t) => {
  const f = await correctionApiFixture(t),
    proxy = await lostResponseProxy(t, f.url, "/v1/money/correction/save");
  const command = commandFor(f);
  await assert.rejects(run(client(f, id(1), f.bearer, proxy.url).saveCorrection(command)), {
    code: "unavailable",
  });
  const result = await run(client(f).cancelCorrection(command));
  assert.equal(result.status, "recorded");
  assert.deepEqual(result.receipt.correction, command.correction);
  assert.equal(f.db.sql("select count(*) from public.financial_events"), "2");
  assert.equal(f.db.sql("select count(*) from public.nest_correction_save_cancellations"), "0");
});

...
for (const approved of [true, false]) {
  test(`correction ${approved ? "confirm" : "decline"} recovers committed response loss across SQLite reopen with no automatic write`, async (t) => {
    ...
    let sends = 0;
    const client = {
      correctionContext: (target) =>
        raw.correctionContext(target).pipe(Effect.provideService(Fetch.Fetch, fetch)),
      correctionApproval: (target) =>
        raw.correctionApproval(target).pipe(Effect.provideService(Fetch.Fetch, fetch)),
      decideCorrection: (input) => {
        sends++;
        return raw.decideCorrection(input).pipe(Effect.provideService(Fetch.Fetch, fetch));
      },
    };
    ...
    assert.deepEqual(await run(local.store.readCorrectionApproval(session, approvalId)), {
      approvalId,
      operationId,
      approved,
    });
    ...
    const recovered = new CorrectionApprovalRuntime(
      correctionApprovalOperations({ store: reopened.store, session }, client),
      approvalId,
    );
    ...
    assert.equal(sends, 1);
    assert.equal(await run(reopened.store.readCorrectionApproval(session, approvalId)), null);

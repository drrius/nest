...
import { files, id, json, mealContext } from "../database/ai-meal-reminder-fixture.mjs";
...
test("registered meal reminder tools share authorized native save and retain exact retries", async (t) => {
  const f = await postgrestFixture(t, [...files, "tests/integration/food-postgrest.sql"]);
  const { input } = mealContext(f.db);
  const turn = {
  ...
  const before = await tools.readMealReminder.execute(
  ...
  const saved = await tools.saveMealReminder.execute(input, call);
  assert.equal(saved.ok, true);
  assert.deepEqual(await tools.saveMealReminder.execute(input, call), saved);
  const read = await tools.readMealReminder.execute(
    { entryId: input.entryId },
  ...
  assert.equal(card.label, "Meal reminder settings saved");
  assert.deepEqual(card.href, {
    pathname: "/meal-reminder",
    params: { entryId: input.entryId },
  });
  assert.equal(f.db.sql("select count(*) from public.nest_ai_commands"), "1");
  assert.equal(f.db.sql("select count(*) from public.financial_events"), "0");
  assert.equal(
    f.db.sql(`select title_snapshot from public.meal_plan_entries where id='${input.entryId}'`),
    "Synthetic soup",
  );
});
...
test("registered reminder reads reject outsiders and injected authority without journal writes", async (t) => {
  ...
  const outsider = await build(f.otherBearer).readMealReminder.execute(
    { entryId: input.entryId },
    call,
  );
  assert.deepEqual(outsider, { ok: false, code: "forbidden" });
  const injected = await build(f.bearer).readMealReminder.execute(
    { entryId: input.entryId, householdId: id(20) },
    call,
  );
  assert.equal(injected.ok, false);
  const partner = await build(f.partnerBearer).readMealReminder.execute(
    { entryId: input.entryId },
    call,
  );

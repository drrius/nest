...
import { fixture, id, run } from "./recurring-reminder-native-fixture.mjs";
test("reminder save survives SQLite restart and recovers the committed result without resending", async (t) => {
  ...
  assert.deepEqual(
    (await run(f.local.store.readRecurringReminderSave(f.session))).command,
    f.command,
  );
  ...
  assert.equal(await run(reopened.store.readRecurringReminderSave(f.session)), null);
  assert.equal(f.sends(), 1);
...
test("account replacement hides pending reminder intent and prevents sending it", async (t) => {
  ...
  assert.equal(await run(f.local.store.readRecurringReminderSave(partner)), null);
...
test("SQLite staging failure sends nothing and a cleanup failure preserves the confirmed reminder for recovery", async (t) => {
  ...
  f.local.connection.exec(
    "CREATE TRIGGER block_stage BEFORE INSERT ON recurring_reminder_save_attempts BEGIN SELECT RAISE(ABORT,'staging failure'); END",
  );
  await runtime.save(f.command);
  ...
  f.local.connection.exec(
    "CREATE TRIGGER block_cleanup BEFORE DELETE ON recurring_reminder_save_attempts BEGIN SELECT RAISE(ABORT,'cleanup failure'); END",
  );
  await runtime.refresh();
  await runtime.save(f.command);
  ...
  assert.equal(runtime.getSnapshot().attempt, null);
  assert.equal(f.sends(), 1);
  assert.equal(await run(f.local.store.readRecurringReminderSave(f.session)), null);
});
test("a paused recurring rule cannot strand its unconfirmed reminder: retry conflicts and cancellation remains available", async (t) => {
  const f = await fixture(t),
    runtime = await f.mount();
  f.fault("before");
  await runtime.save(f.command);
  f.db.sql(`update public.nest_recurring_rules set status='paused' where id='${f.ruleId}'`);
  assert.equal((await run(f.native.detail(f.ruleId))).rule.status, "paused");
  f.fault("none");
  await runtime.refresh();

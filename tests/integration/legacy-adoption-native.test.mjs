...
test("native adoption stages before dispatch and recovers a lost committed response after write suspension and SQLite restart without resending", async (t) => {
  ...
  f.db.sql(`revoke execute on function public.nest_save_legacy_adoption(uuid,uuid,jsonb),
    public.nest_cancel_legacy_adoption(uuid,uuid) from public,anon,authenticated,service_role`);
  runtime.dispose();
  await f.local.idle();

...
function clients(f) {
  ...
  return {
    renewals: renewalClient(f.url, account, credentials),
...
test("reminder editor loads authorized choices and confirms exact settings through durable saving", async (t) => {
  ...
  const context = await run(reminderEditorContext(account, api, id(900)));
  ...
  const loaded = await run(reminderEditorContext(account, api, id(900)));
  ...
  await assert.rejects(run(reminderEditorContext(account, api, id(900))));

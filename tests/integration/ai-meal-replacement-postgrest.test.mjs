...
import { aiMealReplacementFiles } from "../database/ai-meal-replacement-files.mjs";
...
async function setup(t) {
  ...
  const turn = {
    ...
    operationId: id(901),
    expectedRevision: "0",
    text: "Replace Tuesday lunch with lentil soup",
  };
  const message = {
    id: turn.operationId,
    role: "user",
...
test("SDK replacement recovers its original receipt without undoing a partner edit", async (t) => {
  ...
  assert.deepEqual(await tools.replaceMeal.execute(value, options("place")), {
    ok: false,
    code: "unavailable",
  });
  assert.equal(f.proxy.dropped(), 1);
  assert.deepEqual(await tools.replaceMeal.execute(value, options("reremoval")), {
    ok: false,
    code: "unavailable",
  });
  const receipt = JSON.parse(
    f.remote.db.sql("select result from public.nest_meal_replacement_receipts"),
  );
  f.remote.db.sql(
    `update public.meal_plan_entries set title_snapshot='Partner soup' where id='${receipt.entryId}'`,
  );
  assert.equal(receipt.previousEntryId, value.entryId.toLowerCase());
  assert.equal(receipt.revision, "3");
  const retry = await f.connect(true).replaceMeal.execute(value, options("place"));
  assert.deepEqual(retry, { ok: true, value: receipt });
  const fresh = await f
    .connect()
    .readMealWeek.execute({ weekStart: value.weekStart }, options("fresh"));
  assert.equal(fresh.value.revision, "4");
  assert.equal(fresh.value.entries[0].entryId, receipt.entryId);
  assert.equal(fresh.value.entries[0].title, "Partner soup");
  assert.equal(f.remote.db.sql("select count(*) from public.nest_meal_replacement_receipts"), "1");
  assert.equal(f.remote.db.sql("select count(*) from public.nest_ai_commands"), "1");
  assert.deepEqual(
    await f.connect(false, f.remote.partnerBearer).replaceMeal.execute(value, options("place")),
...
test("SDK input identities are rejected and stale week conflicts cannot be silently retried as writes", async (t) => {
  ...
  for (const patch of [{ actorId: id(2) }, { operationId: id(999) }, { householdId: id(20) }])
    assert.deepEqual(
      await f.connect().replaceMeal.execute({ ...value, ...patch }, options("invalid")),
  ...
  assert.deepEqual(await f.connect().replaceMeal.execute(value, options("stale")), {
    ok: false,
    code: "conflict",
  });
  ...
  assert.equal(f.remote.db.sql("select count(*) from public.nest_meal_replacement_receipts"), "0");
});

...
import { input } from "../database/recipe-archive-fixture.mjs";
const value = input();
...
async function setup(t) {
  ...
  const turn = {
    ...
    operationId: id(901),
    expectedRevision: "0",
    text: "Archive the saved soup recipe",
  };
  const message = {
    id: turn.operationId,
    role: "user",
...
test("SDK archive recovers a lost journal response without undoing a later restoration", async (t) => {
  ...
  assert.deepEqual(await tools.archiveRecipe.execute(value, options("archive")), {
    ok: false,
    code: "unavailable",
  });
  assert.equal(f.proxy.dropped(), 1);
  assert.deepEqual(await tools.archiveRecipe.execute(value, options("new-call")), {
    ok: false,
    code: "unavailable",
  });
  const receipt = JSON.parse(
    f.remote.db.sql("select result from public.nest_recipe_archive_receipts"),
  );
  f.remote.db.sql(
    `update public.meal_definitions set name='Restored soup',archived_at=null where id='${receipt.definitionId}'`,
  );
  assert.deepEqual(await f.connect(true).archiveRecipe.execute(value, options("archive")), {
    ok: true,
    value: receipt,
  });
  assert.equal(f.remote.db.sql("select count(*) from public.nest_ai_commands"), "1");
  assert.equal(f.remote.db.sql("select count(*) from public.nest_recipe_archive_receipts"), "1");
  ...
  assert.deepEqual(
    await f.connect(false, f.remote.partnerBearer).archiveRecipe.execute(value, options("archive")),
    { ok: false, code: "forbidden" },
  );
  f.remote.db.sql(`delete from public.household_members where user_id='${id(1)}'`);
  assert.deepEqual(await f.connect().archiveRecipe.execute(value, options("archive")), {
    ok: false,
    code: "forbidden",
  });
});
test("SDK stale archive records a conflict and blocks further writes in the turn", async (t) => {
  const f = await setup(t),
    tools = f.connect();
  f.remote.db.sql(
    `update public.meal_definitions set name='Partner edit' where id='${value.definitionId}'`,
  );
  assert.deepEqual(await tools.archiveRecipe.execute(value, options("stale")), {
    ok: false,
    code: "conflict",
  });
  assert.deepEqual(
    await tools.archiveRecipe.execute({ ...value, expectedRevision: "1" }, options("new-call")),
    { ok: false, code: "unavailable" },
  );
  assert.deepEqual(await f.connect().archiveRecipe.execute(value, options("stale")), {
    ok: false,
    code: "conflict",
  });
  assert.equal(f.remote.db.sql("select count(*) from public.nest_recipe_archive_receipts"), "0");
  assert.equal(f.remote.db.sql("select count(*) from public.nest_ai_commands"), "1");
});

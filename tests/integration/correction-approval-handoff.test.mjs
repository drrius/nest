...
import { correction as payload, replacement } from "./correction-api-fixture.mjs";
...
async function proposal(f, config, change) {
  const turn = {
  ...
  return tools.proposeCorrection.execute(payload(f.source, { replacement: change }), {
    toolCallId: "correction",
    messages: [],
  });
}
for (const change of [null, replacement()]) {
  test(`real AI ${change ? "replacement" : "reversal"} card opens native review and records only after exact explicit confirmation`, async (t) => {
    ...
    const result = await proposal(f, config, change);
    ...
    assert.equal(card.href.pathname, "/correction-approval");
    ...
    const client = {
      correctionContext: (target) => provided(raw.correctionContext(target)),
      correctionApproval: (target) => provided(raw.correctionApproval(target)),
      decideCorrection: (input) => provided(raw.decideCorrection(input)),
    };
    const owner = correctionApprovalOwner(
      correctionApprovalOperations({ store: local.store, session }, client),
    ...
    assert.match(
      correctionConfirmation(view.approval, view.context, id(1)),
    ...
    assert.equal(f.db.sql("select count(*) from public.financial_events"), change ? "3" : "2");
    assert.equal(f.db.sql("select count(*) from public.nest_correction_receipts"), "1");
    stop();
  });
}

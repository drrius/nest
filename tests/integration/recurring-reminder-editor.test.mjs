...
import { fixture, id, run, Effect } from "./recurring-reminder-native-fixture.mjs";
import { routineClient } from "../../apps/mobile/src/routines/client.ts";
import { reminderEditorContext } from "../../apps/mobile/src/recurring-reminders/editor-context.ts";
import {
  reminderDraft,
  parseReminderDraft,
} from "../../apps/mobile/src/recurring-reminders/form.ts";
import { reminderConfirmation } from "../../apps/mobile/src/recurring-reminders/confirmation.ts";
function clients(f) {
...
test("reminder editor loads authorized choices and confirms exact settings through durable saving", async (t) => {
  ...
  const context = await run(reminderEditorContext(account, api, f.ruleId));
  ...
  const loaded = await run(reminderEditorContext(account, api, f.ruleId));
  ...
  await assert.rejects(run(reminderEditorContext(account, api, f.ruleId)));

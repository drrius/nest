...
import { files, id, json, payload } from "../database/ai-manual-cycle-fixture.mjs";
...
import { manualCycleApprovalOwner } from "../../apps/mobile/src/money/recurring-manual-approval-owner.ts";
...
async function proposal(f, config) {
  const turn = {
    ...
    operationId: id(901),
    expectedRevision: "0",
    text: "Link the explicitly selected existing expense to this due recurring cycle",
  };
  const message = {
    id: turn.operationId,
    role: "user",
  ...
  const result = await tools.proposeManualCycle.execute(input, {
    toolCallId: "recurring",
    messages: [],
  });
  assert.deepEqual(
    await tools.proposeManualCycle.execute(input, { toolCallId: "recurring", messages: [] }),
    result,
  );
  return result;
}
test("real AI tool returns a private native handoff; only exact confirmation records a manual cycle", async (t) => {
  ...
  assert.equal(card.href.pathname, "/recurring-manual-approval");
  assert.match(card.label, /not linked yet/);
  ...
  const client = Object.fromEntries(
    ["manualCycleApproval", "manualCycleContext", "decideManualCycle"].map((name) => [
      name,
      (...args) => raw[name](...args).pipe(Effect.provideService(Fetch.Fetch, fetch)),
    ]),
  );
  const owner = manualCycleApprovalOwner(

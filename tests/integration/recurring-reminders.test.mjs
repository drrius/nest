...
import { fixture, id, run, Effect, Fetch } from "./recurring-reminder-fixture.mjs";
test("recurring reminder native HTTP recovers a lost committed response and refuses substituted intent", async (t) => {
  const f = await fixture(t),
    command = f.command;
  assert.equal((await run(f.native.detail(f.ruleId))).reminder, null);
...
test("recurring reminder transport rejects foreign and stale writes, query injection and cancelled sends", async (t) => {
  const f = await fixture(t),
    command = f.command;
  ...
  for (const query of [
    `detail?ruleId=${f.ruleId}&ruleId=${f.ruleId}`,
    `operation?operationId=${id(2000)}&extra=1`,
  ]) {
    const response = await fetch(`${f.url}/v1/recurring-reminders/${query}`, {
      headers: { authorization: `Bearer ${f.bearer}`, "x-nest-household": id(10) },
    });
    assert.equal(response.status, 400);
  }
});
test("recurring reminder native client rejects forged ownership, receipt intent and target context", async (t) => {
  const f = await fixture(t),
    command = f.command;
  ...
  const context = await run(f.native.detail(f.ruleId));
  const forged = { ...context, rule: { ...context.rule, ruleId: id(999) }, reminder: null };
  await assert.rejects(
    run(
      f.native
        .detail(f.ruleId)

...
test("private native card stages before dispatch and recovers a committed dismissal after SQLite restart without replay", async (t) => {
  ...
  assert.match(
    legacyDismissalApprovalText(approval, runtime.getSnapshot().context, id(1)),
    /Original draft.*No expense or payment/s,
  );
  f.fault("after");
...
test("withdrawal truthfully recovers a dismissal that won first and expiry still permits explicit denial", async (t) => {
  const f = await fixture(t),
    runtime = await f.mount();
  f.fault("before");
  await runtime.decide(runtime.getSnapshot().approval, true);
  await runtime.refresh();
  await run(f.native.decideLegacyDismissal(f.decision));
  f.fault("none");

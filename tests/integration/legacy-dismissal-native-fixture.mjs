...
import { legacyDismissalSaveOperations } from "../../apps/mobile/src/money/legacy-dismissal-save-operations.ts";
import { LegacyDismissalSaveRuntime } from "../../apps/mobile/src/money/legacy-dismissal-save-runtime.ts";
export { id, run };
...
export async function fixture(t) {
  ...
  const transport = async (input, init) => {
    const save = String(input).includes("/legacy-dismissal/save");
    if (save) {
      sends++;
  ...
  const client = Object.fromEntries(
    [
      "legacyDraftContext",
      "saveLegacyDismissal",
      "recoverLegacyDismissal",
      "cancelLegacyDismissal",
    ].map((key) => [
      key,
  ...
  const operations = (store = local.store) =>
    legacyDismissalSaveOperations({ store, session }, client);
  const mount = async (store = local.store) => {
    const runtime = new LegacyDismissalSaveRuntime(operations(store));

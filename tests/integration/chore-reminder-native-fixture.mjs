...
export async function fixture(t) {
  ...
  const transport = async (input, init) => {
    const save = String(input).endsWith("/chore-reminders/save");
    if (save) {
      sends++;
  ...
  const operations = (store = local.store) =>
    choreReminderSaveOperations({ store, session }, client);
  const mount = async (store = local.store) => {
    const runtime = new ChoreReminderSaveRuntime(operations(store));

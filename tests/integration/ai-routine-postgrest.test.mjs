...
test("actual SDK tool execution recovers journaled routine creation through PostgREST after lost response", async (t) => {
  ...
  const tools = connect();
  assert.deepEqual(await tools.createRoutine.execute(input, options), {
    ok: false,
    code: "unavailable",
  });
  ...
  const recovered = await connect().createRoutine.execute(input, options);
  assert.equal(recovered.ok, true);
  assert.equal(recovered.value.action, "create");
  assert.equal(remote.db.sql("select count(*) from public.routines"), "1");
  ...
  assert.deepEqual(await partner.createRoutine.execute(input, options), {
    ok: false,
    code: "forbidden",
  });

...
async function proposal(f, config) {
  ...
  return tools.proposeExpense.execute(
    payload({ categoryId: id(500), receiptPath: f.receiptPath }),
    {
      toolCallId: "expense",
      messages: [],
    },
...
test("actual SDK and native approval preserve the exact uploaded receipt through API and PostgreSQL", async (t) => {
  ...
  f.receiptPath = `${id(10)}/receipts/${id(700)}.jpg`;
  f.db.sql(
    `set role authenticated; set request.jwt.claims='${JSON.stringify({ sub: id(1) })}'; select public.reserve_household_attachment('${f.receiptPath}','image/jpeg')`,
  );
  f.db.sql(
    `insert into storage.objects(bucket_id,name,metadata) values('household-files','${f.receiptPath}','{"mimetype":"image/jpeg","size":128}')`,
  );
  ...
  assert.equal(runtime.getSnapshot().approval.receipt.expense.receiptPath, f.receiptPath);
  assert.equal(f.db.sql("select receipt_path from public.financial_events"), f.receiptPath);
  assert.equal(f.db.sql("select state from public.household_attachment_uploads"), "claimed");
  stop();
});

...
import { fixture, id } from "../database/recurring-push-fixture.mjs";
...
test("recurring uses the actual server RPC worker, shared receipt polling and privacy-safe provider payload", async (t) => {
  const f = fixture(t);
  ...
  const provider = expoPushTransport(undefined, async (url, input) => {
    if (url.endsWith("/send")) {
      ...
      return Response.json({ data: { status: "ok", id: "recurring-http-ticket" } });
    }
    return Response.json({ data: { "recurring-http-ticket": { status: "ok" } } });
  });

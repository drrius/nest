...
test("reminder editor loads authorized choices and confirms exact settings through durable saving", async (t) => {
  ...
  const context = await run(reminderEditorContext(account, api, f.itemId));
  ...
  const loaded = await run(reminderEditorContext(account, api, f.itemId));
  ...
  await assert.rejects(run(reminderEditorContext(account, api, f.itemId)));

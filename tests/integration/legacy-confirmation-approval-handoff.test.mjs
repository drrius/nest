...
import { files, id, json, payload } from "../database/ai-legacy-confirmation-fixture.mjs";
...
import { legacyConfirmationApprovalOwner } from "../../apps/mobile/src/money/legacy-confirmation-approval-owner.ts";
import { legacyConfirmationApprovalActions } from "../../apps/mobile/src/money/legacy-confirmation-approval-display.ts";
...
async function proposal(f, config) {
  const turn = {
    ...
    operationId: id(901),
    expectedRevision: "0",
    text: "Propose the selected retained draft as an expense with complete reviewed terms",
  };
  const message = {
    id: turn.operationId,
    role: "user",
  ...
  const result = await tools.proposeLegacyConfirmation.execute(input, {
    toolCallId: "recurring",
    messages: [],
  });
  assert.deepEqual(
    await tools.proposeLegacyConfirmation.execute(input, { toolCallId: "recurring", messages: [] }),
    result,
  );
  return result;
}
test("real AI tool returns a private native handoff; only exact native confirmation posts the retained expense", async (t) => {
  ...
  assert.equal(card.href.pathname, "/legacy-confirmation-approval");
  assert.match(card.label, /not recorded yet/);
  ...
  const client = Object.fromEntries(
    ["legacyConfirmationApproval", "legacyConfirmationContext", "decideLegacyConfirmation"].map(
      (name) => [
        name,
  ...
  const owner = legacyConfirmationApprovalOwner(
    legacyConfirmationApprovalOperations({ store: local.store, session }, client),
  ...
  assert.equal(legacyConfirmationApprovalActions(view, Date.now()).confirm, true);

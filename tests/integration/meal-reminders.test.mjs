...
import { fixture, id, run, Effect, Fetch } from "./meal-reminder-fixture.mjs";
test("meal reminder native HTTP recovers a lost committed response and refuses substituted intent", async (t) => {
  const f = await fixture(t),
    command = f.command;
  assert.equal((await run(f.native.detail(f.entryId))).reminder, null);
...
test("meal reminder transport rejects foreign and stale writes, query injection and cancelled sends", async (t) => {
  const f = await fixture(t),
    command = f.command;
  ...
  for (const query of [
    `detail?entryId=${f.entryId}&entryId=${f.entryId}`,
    `operation?operationId=${id(2000)}&extra=1`,
  ]) {
    const response = await fetch(`${f.url}/v1/meal-reminders/${query}`, {
      headers: { authorization: `Bearer ${f.bearer}`, "x-nest-household": id(10) },
    });
    assert.equal(response.status, 400);
  }
});
test("meal reminder native client rejects forged ownership, receipt intent and target context", async (t) => {
  const f = await fixture(t),
    command = f.command;
  ...
  const context = await run(f.native.detail(f.entryId));
  const forged = { ...context, meal: { ...context.meal, entryId: id(999) }, reminder: null };
  await assert.rejects(
    run(
      f.native
        .detail(f.entryId)

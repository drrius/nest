...
import { aiMealPlacementFiles } from "../database/ai-meal-placement-files.mjs";
...
async function setup(t) {
  ...
  const turn = {
    ...
    operationId: id(901),
    expectedRevision: "0",
    text: "Add pasta for Tuesday lunch",
  };
  const message = {
    id: turn.operationId,
    role: "user",
...
test("SDK placement recovers a committed lost response without recreating a partner-removed meal", async (t) => {
  ...
  assert.deepEqual(await tools.placeMeal.execute(value, options("place")), {
    ok: false,
    code: "unavailable",
  });
  assert.equal(f.proxy.dropped(), 1);
  assert.deepEqual(await tools.placeMeal.execute(value, options("replacement")), {
    ok: false,
    code: "unavailable",
  });
  ...
  f.remote.db.sql(
    `update public.meal_plan_entries set removed_at=now() where id='${receipt.entryId}'`,
  );
  const retry = await f.connect(true).placeMeal.execute(value, options("place"));
  ...
  assert.deepEqual(
    await f.connect(false, f.remote.partnerBearer).placeMeal.execute(value, options("place")),
...
test("SDK input identities are rejected and stale week conflicts cannot be silently retried as writes", async (t) => {
  ...
  for (const patch of [{ actorId: id(2) }, { operationId: id(999) }, { householdId: id(20) }])
    assert.deepEqual(
      await f.connect().placeMeal.execute({ ...value, ...patch }, options("invalid")),
  ...
  assert.deepEqual(await f.connect().placeMeal.execute(value, options("stale")), {
    ok: false,
    code: "conflict",
  });

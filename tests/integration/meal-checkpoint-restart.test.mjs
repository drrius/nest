...
import { mealPushRpc } from "../../apps/api/src/push/meal-rpc.ts";
...
test("lost committed meal checkpoint resumes the next page after restart and never resends", async (t) => {
  const f = await summaryWorkerFixture(t, mealFixture);
  f.rpc = mealPushRpc(f.baseRpc);
  f.db

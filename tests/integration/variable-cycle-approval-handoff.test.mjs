...
import { files, id, json, payload } from "../database/ai-variable-cycle-fixture.mjs";
...
async function proposal(f, config) {
  const turn = {
    ...
    operationId: id(901),
    expectedRevision: "0",
    text: "Record this due variable bill with the explicit CHF 1.01 amount and split",
  };
  const message = {
    id: turn.operationId,
    role: "user",
  ...
  const result = await tools.proposeVariableCycle.execute(input, {
    toolCallId: "recurring",
    messages: [],
  });
  assert.deepEqual(
    await tools.proposeVariableCycle.execute(input, { toolCallId: "recurring", messages: [] }),
    result,
  );
  return result;
}
test("real AI tool returns a private native handoff; only exact confirmation records a variable cycle", async (t) => {
  ...
  assert.equal(card.href.pathname, "/recurring-variable-approval");
  assert.match(card.label, /no expense recorded/);
  ...
  const client = Object.fromEntries(
    ["variableCycleApproval", "recurringRule", "decideVariableCycle"].map((name) => [
      name,
      (...args) => raw[name](...args).pipe(Effect.provideService(Fetch.Fetch, fetch)),
    ]),
  );
  const owner = variableCycleApprovalOwner(

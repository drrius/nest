...
for (const approved of [true, false]) {
  test(`recurring resumption ${approved ? "approval" : "denial"} recovers lost acknowledgment through fresh native client`, async (t) => {
    ...
    const pending = await run(client.recurringResumeApproval(approvalId));
    ...
    await assert.rejects(run(f.client(proxy.url).decideRecurringResume(input)));
    assert.equal(proxy.dropped(), 1);
    const recovered = await run(f.client().recurringResumeApproval(approvalId));
    assert.equal(recovered.status, approved ? "consumed" : "denied");
    assert.deepEqual(await run(f.client().decideRecurringResume(input)), recovered);
    ...
    await assert.rejects(
      run(f.client(f.url, 2, f.partnerBearer).recurringResumeApproval(approvalId)),
    );
    await assert.rejects(run(client.decideRecurringResume({ ...input, operationId: id(701) })));
    const duplicate = await fetch(
      `${f.url}/v1/money/recurring/resume/approval?approvalId=${approvalId}&approvalId=${approvalId}`,
      {
        headers: { authorization: `Bearer ${f.bearer}`, "x-nest-household": id(10) },
      },
    );
    assert.equal(duplicate.status, 400);
    const extra = await f.send("/v1/money/recurring/resume/approval/decide", {
      ...input,
      actorId: id(1),
    });
    assert.equal(extra.status, 400);
    f.db.sql(`delete from public.household_members where user_id='${id(1)}'`);
    await assert.rejects(run(client.recurringResumeApproval(approvalId)));
    await assert.rejects(run(client.decideRecurringResume(input)));
  });
}
...
async function substitutedResponses(client, approvalId, pending) {
  ...
  for (const change of [
    ...
    await assert.rejects(
      Effect.runPromise(
        client.recurringResumeApproval(approvalId).pipe(

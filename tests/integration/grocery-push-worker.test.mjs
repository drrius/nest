...
import { fixture, id } from "../database/grocery-push-fixture.mjs";
...
test("grocery uses the actual server RPC worker, shared receipt polling and privacy-safe provider payload", async (t) => {
  const f = fixture(t);
  ...
  const provider = expoPushTransport(undefined, async (url, input) => {
    if (url.endsWith("/send")) {
      ...
      return Response.json({ data: { status: "ok", id: "grocery-http-ticket" } });
    }
    return Response.json({ data: { "grocery-http-ticket": { status: "ok" } } });
  });

...
import { files, id, json, payload } from "../database/ai-recurring-resume-fixture.mjs";
...
async function proposal(f, config) {
  const turn = {
    ...
    operationId: id(901),
    expectedRevision: "0",
    text: "Resume this paused recurring expense",
  };
  const message = {
    id: turn.operationId,
    role: "user",
  ...
  const result = await tools.proposeRecurringResume.execute(input, {
    toolCallId: "recurring",
    messages: [],
  });
  assert.deepEqual(
    await tools.proposeRecurringResume.execute(input, { toolCallId: "recurring", messages: [] }),
    result,
  );
  return result;
}
test("real AI tool returns a private native handoff; only exact confirmation resumes a recurring rule", async (t) => {
  ...
  assert.equal(card.href.pathname, "/recurring-resume-approval");
  ...
  const client = Object.fromEntries(
    ["recurringResumeApproval", "recurringRule", "decideRecurringResume"].map((name) => [
      name,

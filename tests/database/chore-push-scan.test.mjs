...
import { fixture, id } from "./chore-push-fixture.mjs";
function setup(t) {
  const f = fixture(t);
  ...
  const scan = (after) =>
    JSON.parse(
      f.db.sql(
        after
          ? `select public.nest_scan_chore_push_deliveries('${after.dueAt}','${after.outboxId}','${after.installationId}')`
          : "select public.nest_scan_chore_push_deliveries()",
      ),
    );
  return { ...f, scan };
}
test("chore scan returns token-free prepared identities and begin rechecks changed authorization", (t) => {
  const f = setup(t);
  const page = f.scan();
...
test("scan enforces complete cursors and server-only access", (t) => {
  const f = setup(t);
  assert.throws(
    () => f.db.sql(`select public.nest_scan_chore_push_deliveries(null,'${f.choreOutbox}',null)`),
    /Invalid push scan cursor/,
  );
  assert.throws(
    () =>
      f.db.sql(
        `select public.nest_scan_chore_push_deliveries('infinity','${f.choreOutbox}','${id(1702)}')`,
      ),
    /Invalid push scan cursor/,
  );
  for (const role of ["anon", "authenticated"])
    assert.throws(
      () => f.db.sql(`set role ${role}; select public.nest_scan_chore_push_deliveries()`),
      /permission denied/,
    );
  assert.equal(
    JSON.parse(f.db.sql("set role service_role; select public.nest_scan_chore_push_deliveries()"))
      .scanned,
    1,
  );
});
test("chore scan excludes cancelled or expired rows and rolls preparation back atomically", (t) => {
  const f = setup(t);
  f.db.sql("begin; select public.nest_scan_chore_push_deliveries(); rollback");
  assert.equal(f.db.sql("select count(*) from private.nest_push_deliveries"), "0");
  f.db.sql(
    "update private.nest_chore_reminder_outbox set due_at=clock_timestamp()-interval '2 days'",
  );
  assert.equal(f.scan().scanned, 0);
  f.db.sql(
    "update private.nest_chore_reminder_outbox set due_at=clock_timestamp(),state='cancelled'",
  );
  assert.equal(f.scan().scanned, 0);
});

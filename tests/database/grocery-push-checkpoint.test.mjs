...
function setup(t) {
  ...
  const read = () =>
    JSON.parse(
      f.db.sql("set role service_role; select public.nest_read_grocery_push_checkpoint()"),
    );
  const saveSql = (revision, after) =>
    `set role service_role; select public.nest_save_grocery_push_checkpoint('${revision}',${json(after)})`;
...
test("malformed cursor cannot advance progress and checkpoint state is private", (t) => {
  ...
  for (const role of ["anon", "authenticated"])
    assert.throws(
      () => f.db.sql(`set role ${role}; select public.nest_read_grocery_push_checkpoint()`),
      /permission denied/,
    );
  assert.throws(
    () =>
      f.db.sql("set role service_role; select * from private.nest_grocery_push_scan_checkpoint"),
    /permission denied/,
  );
});

test("server maintenance materializes fixed windows and cancels muted groceries without releasing tokens", (t) => {
  const f = setup(t);
  f.db.sql(
    "delete from private.nest_grocery_reminder_outbox; delete from private.nest_grocery_reminder_scans",
  );
  const maintain = () =>
    JSON.parse(f.db.sql("set role service_role; select public.nest_maintain_grocery_reminders()"));
  ...
  assert.equal(f.db.sql("select count(*) from private.nest_grocery_reminder_scans"), "2");
  ...
  for (const role of ["anon", "authenticated"])
    assert.throws(
      () => f.db.sql(`set role ${role}; select public.nest_maintain_grocery_reminders()`),
      /permission denied/,
    );
});

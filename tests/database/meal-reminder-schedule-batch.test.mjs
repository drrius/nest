...
import { setup, id } from "./meal-reminder-schedule-fixture.mjs";
test("source cursor progresses past 250 retained deleted items and wraps for later changes", (t) => {
  const f = setup(t);
  f.db
    .sql(`insert into public.nest_meal_reminders(household_id,entry_id,revision,reviewed_item_revision,updated_by,settings)
    select household_id,('00000000-0000-0000-0000-'||lpad(n::text,12,'0'))::uuid,gen_random_uuid(),reviewed_item_revision,updated_by,settings
    from public.nest_meal_reminders cross join generate_series(10000,10249) n`);
...
test("cancellation cursor limits inspected rows to 500 and reaches later obsolete work", (t) => {
  const f = setup(t);
  f.db
    .sql(`insert into private.nest_meal_reminder_outbox(household_id,entry_id,item_revision,schedule_revision,recipient_id,due_at)

...
function setup(t) {
  ...
  f.db.file("supabase/migrations/20260923005723_native_push_delivery_scan.sql");
  const scan = (after) =>
    JSON.parse(
      f.db.sql(
        after
          ? `select public.nest_scan_push_deliveries('${after.dueAt}','${after.outboxId}','${after.installationId}')`
          : "select public.nest_scan_push_deliveries()",
      ),
    );
  return { ...f, scan };
}
test("scan returns token-free prepared identities and begin rechecks changed authorization", (t) => {
  const f = setup(t);
  const page = f.scan();
...
test("scan enforces complete cursors and server-only access", (t) => {
  const f = setup(t);
  assert.throws(
    () => f.db.sql(`select public.nest_scan_push_deliveries(null,'${f.outbox}',null)`),
    /Invalid push scan cursor/,
  );
  assert.throws(
    () =>
      f.db.sql(`select public.nest_scan_push_deliveries('infinity','${f.outbox}','${id(1702)}')`),
    /Invalid push scan cursor/,
  );
  for (const role of ["anon", "authenticated"])
    assert.throws(
      () => f.db.sql(`set role ${role}; select public.nest_scan_push_deliveries()`),
      /permission denied/,
    );
  assert.equal(
    JSON.parse(f.db.sql("set role service_role; select public.nest_scan_push_deliveries()"))
      .scanned,
    1,
  );
});

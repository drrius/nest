...
import { fixture as base, id, as, command, approve } from "./native-refund-fixture.mjs";
function fixture(t) {
  const f = base(t);
  f.db.file("supabase/migrations/20260921152631_native_refund_save_cancel.sql");
  return f;
}
const cancel = (op) => `select public.nest_cancel_refund_save('${id(10)}','${id(op)}')`;
const read = (op) => `select public.nest_read_refund_save('${id(10)}','${id(op)}')`;
test("refund cancellation is permanent, private, immutable and does not modify history", async (t) => {
  const f = fixture(t),
    payload = f.payload();
  ...
  assert.equal(f.db.sql(as(2, "select count(*) from public.nest_refund_save_cancellations")), "0");
  assert.throws(() => f.record(cancel(101), 3), /Not authorized/);
  assert.throws(() => f.db.sql(`set role anon; ${cancel(101)}`), /permission denied/);
  assert.throws(
    () => f.db.sql(as(1, "delete from public.nest_refund_save_cancellations")),
    /permission denied/,
  );
  assert.throws(() => f.db.sql("delete from public.nest_refund_save_cancellations"), /append-only/);
  assert.throws(() => f.record(read(100), 3), /Not authorized/);
});
test("20 refund Save/cancel races converge without reversing a recorded refund", async (t) => {
  const f = fixture(t);
...
test("cancellation cannot approve or reverse AI refunds and a failed marker remains unresolved", (t) => {
  const f = fixture(t),
    payload = f.payload();
  f.db.sql(
    "alter table public.nest_refund_save_cancellations add constraint fixture_failure check(false) not valid",
  );
  assert.throws(() => f.record(cancel(100)), /fixture_failure/);
  assert.equal(f.record(read(100)).status, "unresolved");
  f.db.sql("alter table public.nest_refund_save_cancellations drop constraint fixture_failure");

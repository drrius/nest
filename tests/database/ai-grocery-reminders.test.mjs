import { matchesGroceryReminderReceipt } from "../../apps/api/src/grocery-reminders/matches-receipt.ts";
import test from "node:test";
import assert from "node:assert/strict";
import { setup, as, id, json } from "./ai-grocery-reminder-fixture.mjs";
test("AI reminder journal delegates native save once under concurrent invocation retries", async (t) => {
  ...
  assert.equal(
    matchesGroceryReminderReceipt("saveGroceryReminder", f.input, results[0].value, member),
    true,
  );
  assert.equal(
    matchesGroceryReminderReceipt(
      "saveGroceryReminder",
  ...
  assert.equal(
    matchesGroceryReminderReceipt("saveGroceryReminder", f.input, results[0].value, {
      ...member,
      userId: id(2),
    }),
    false,
  ...
  assert.equal(f.db.sql("select count(*) from public.nest_grocery_reminders"), "1");
...
test("AI reminder command rejects injected identity, foreign recipients and stale baselines", (t) => {
  ...
  assert.throws(
    ...
    /Invalid grocery reminder command/,
  );
  for (const [i, input] of [
    { ...f.input, expectedItemVersion: "2" },
    {
      ...f.input,
      settings: {
        ...f.input.settings,
...
test("AI journal failure rolls back reminder settings and native operation receipt", (t) => {
  const f = setup(t);
  f.db.sql(
    "alter table public.nest_ai_commands add constraint reject_reminder check(tool_name <> 'saveGroceryReminder')",
  );
  assert.throws(() => f.db.sql(as(f.reminderCommand(f.input))), /reject_reminder/);
  assert.equal(f.db.sql("select count(*) from public.nest_grocery_reminders"), "0");
  assert.equal(f.db.sql("select count(*) from private.nest_grocery_reminder_operations"), "0");
});
test("saved reminder transcript uses committed receipt instead of fabricated delivery claims", (t) => {
  ...
  const parts = transcript.at(-1).parts.filter((part) => part.type === "tool-saveGroceryReminder");
  assert.equal(parts.length, 1);
  assert.deepEqual(parts[0].output, result);
  assert.equal(JSON.stringify(parts).includes('"delivered":true'), false);
});

test("private grocery reminder history cannot cross owners and rejects revoked recipients", (t) => {
  const f = setup(t),
  ...
  f.db.sql(`update public.grocery_items set name='Changed' where id='${f.input.itemId}'`);
  ...
  const context = JSON.parse(
    f.db.sql(as(`select public.nest_read_grocery_reminder('${id(10)}','${f.input.itemId}')`)),
  );
  const changed = {
    ...f.input,

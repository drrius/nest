import { matchesMealReminderReceipt } from "../../apps/api/src/meal-reminders/matches-receipt.ts";
import test from "node:test";
import assert from "node:assert/strict";
import { setup, as, id, json } from "./ai-meal-reminder-fixture.mjs";
test("AI reminder journal delegates native save once under concurrent invocation retries", async (t) => {
  ...
  assert.equal(
    matchesMealReminderReceipt("saveMealReminder", f.input, results[0].value, member),
    true,
  );
  assert.equal(
    matchesMealReminderReceipt(
      "saveMealReminder",
  ...
  assert.equal(
    matchesMealReminderReceipt("saveMealReminder", f.input, results[0].value, {
      ...member,
      userId: id(2),
    }),
    false,
  ...
  assert.equal(f.db.sql("select count(*) from public.nest_meal_reminders"), "1");
...
test("AI reminder command rejects injected identity, foreign recipients and stale baselines", (t) => {
  ...
  assert.throws(
    ...
    /Invalid meal reminder command/,
  );
  for (const [i, input] of [
    { ...f.input, expectedItemRevision: "0".repeat(64) },
    {
      ...f.input,
      settings: {
        ...f.input.settings,
...
test("AI journal failure rolls back reminder settings and native operation receipt", (t) => {
  const f = setup(t);
  f.db.sql(
    "alter table public.nest_ai_commands add constraint reject_reminder check(tool_name <> 'saveMealReminder')",
  );
...
test("saved reminder transcript uses committed receipt instead of fabricated delivery claims", (t) => {
  ...
  const parts = transcript.at(-1).parts.filter((part) => part.type === "tool-saveMealReminder");
  assert.equal(parts.length, 1);
  assert.deepEqual(parts[0].output, result);
  assert.equal(JSON.stringify(parts).includes('"delivered":true'), false);
});

test("private meal reminder history cannot cross owners and rejects revoked recipients", (t) => {
  const f = setup(t),
  ...
  f.db.sql(`update public.meal_plan_entries set date=date+1 where id='${f.input.entryId}'`);
  ...
  const context = JSON.parse(
    f.db.sql(as(`select public.nest_read_meal_reminder('${id(10)}','${f.input.entryId}')`)),
  );
  const changed = {
    ...f.input,
...
test("meal journal migration preserves chore reminder dispatch and canonical history", async (t) => {
  const f = setup(t);
  const { choreContext } = await import("./ai-chore-reminder-fixture.mjs");
  const { input } = choreContext(f.db);
  const sql = `select public.nest_execute_ai_command('${id(10)}','${f.turn.conversation}','${f.turn.turn}','chore-regression','saveChoreReminder',${json(input)})`;
  const saved = JSON.parse(f.db.sql(as(sql)));
  assert.equal(saved.ok, true);
  assert.deepEqual(saved.value.command.settings, input.settings);
  assert.deepEqual(JSON.parse(f.db.sql(as(sql))), saved);

...
import { fixture, id } from "./grocery-push-fixture.mjs";
function setup(t) {
  const f = fixture(t);
  f.db.file("supabase/migrations/20260923054801_native_grocery_push_scan.sql");
  const scan = (after) =>
    JSON.parse(
      f.db.sql(
        after
          ? `select public.nest_scan_grocery_push_deliveries('${after.dueAt}','${after.outboxId}','${after.installationId}')`
          : "select public.nest_scan_grocery_push_deliveries()",
      ),
    );
  return { ...f, scan };
}
test("grocery scan returns token-free prepared identities and begin rechecks changed authorization", (t) => {
  const f = setup(t);
  const page = f.scan();
...
test("scan enforces complete cursors and server-only access", (t) => {
  ...
  assert.throws(
    () =>
      f.db.sql(`select public.nest_scan_grocery_push_deliveries(null,'${f.groceryOutbox}',null)`),
    /Invalid push scan cursor/,
  );
  assert.throws(
    () =>
      f.db.sql(
        `select public.nest_scan_grocery_push_deliveries('infinity','${f.groceryOutbox}','${id(1702)}')`,
      ),
    /Invalid push scan cursor/,
  );
  for (const role of ["anon", "authenticated"])
    assert.throws(
      () => f.db.sql(`set role ${role}; select public.nest_scan_grocery_push_deliveries()`),
      /permission denied/,
    );
  assert.equal(
    JSON.parse(f.db.sql("set role service_role; select public.nest_scan_grocery_push_deliveries()"))
      .scanned,
    1,
  );
});
test("grocery scan excludes cancelled or expired rows and rolls preparation back atomically", (t) => {
  const f = setup(t);
  f.db.sql("begin; select public.nest_scan_grocery_push_deliveries(); rollback");
  assert.equal(f.db.sql("select count(*) from private.nest_push_deliveries"), "0");
  f.db.sql(
    "update private.nest_grocery_reminder_outbox set due_at=clock_timestamp()-interval '2 days'",
  );
  assert.equal(f.scan().scanned, 0);
  f.db.sql(
    "update private.nest_grocery_reminder_outbox set due_at=clock_timestamp(),state='cancelled'",
  );
  assert.equal(f.scan().scanned, 0);
});

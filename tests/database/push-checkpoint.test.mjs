...
function setup(t) {
  ...
  f.db.file("supabase/migrations/20260923010425_native_push_scan_checkpoint.sql");
  const read = () =>
    JSON.parse(f.db.sql("set role service_role; select public.nest_read_push_checkpoint()"));
  const saveSql = (revision, after) =>
    `set role service_role; select public.nest_save_push_checkpoint('${revision}',${json(after)})`;
...
test("malformed cursor cannot advance progress and checkpoint state is private", (t) => {
  ...
  for (const role of ["anon", "authenticated"])
    assert.throws(
      () => f.db.sql(`set role ${role}; select public.nest_read_push_checkpoint()`),
      /permission denied/,
    );
  assert.throws(
    () => f.db.sql("set role service_role; select * from private.nest_push_scan_checkpoint"),
    /permission denied/,
  );
});

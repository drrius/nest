...
test("meal claims release exactly one token and share immutable outcomes with renewal and summary", async (t) => {
  const f = fixture(t),
    delivery = f.prepare();
  ...
  assert.equal(claim.entryId, id(5001));
  assert.equal(claim.outboxId, f.mealOutbox);
  assert.equal(JSON.stringify(claim).includes("Private meal title"), false);
...
test("meal token release rechecks item, recipient, registration and authenticated session", async (t) => {
  for (const mutation of [
    "update public.nest_notification_preferences set item_reminders_enabled=false",
    `update public.meal_plan_entries set title_snapshot='Changed' where id='${id(5001)}'`,
    `update public.meal_plan_entries set removed_at=now() where id='${id(5001)}'`,
    `update public.meal_plan_entries set date=date+1 where id='${id(5001)}'`,
    "update public.nest_meal_reminders set revision=gen_random_uuid()",
...
test("unknown meal sends cannot be resent and invalid device results disable the matching registration", (t) => {
  const f = fixture(t),
    delivery = f.prepare(),
...
test("meal rate-limit retries cap at three and revalidate recipient settings", (t) => {
  const f = fixture(t),
    delivery = f.prepare();
  f.db.sql(
...
test("meal invalid-token receipt disables only the exact recipient registration", (t) => {
  const f = fixture(t),
    delivery = f.prepare(),
    claim = f.begin(delivery);
  f.db.sql(

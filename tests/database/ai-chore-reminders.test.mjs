import { matchesChoreReminderReceipt } from "../../apps/api/src/chore-reminders/matches-receipt.ts";
import test from "node:test";
import assert from "node:assert/strict";
import { setup, as, id, json } from "./ai-chore-reminder-fixture.mjs";
test("AI reminder journal delegates native save once under concurrent invocation retries", async (t) => {
  ...
  assert.equal(
    matchesChoreReminderReceipt("saveChoreReminder", f.input, results[0].value, member),
    true,
  );
  assert.equal(
    matchesChoreReminderReceipt(
      "saveChoreReminder",
  ...
  assert.equal(
    matchesChoreReminderReceipt("saveChoreReminder", f.input, results[0].value, {
      ...member,
      userId: id(2),
    }),
    false,
...
test("AI reminder command rejects injected identity, foreign recipients and stale baselines", (t) => {
  ...
  assert.throws(
    ...
    /Invalid chore reminder command/,
  );
...
test("AI journal failure rolls back reminder settings and native operation receipt", (t) => {
  const f = setup(t);
  f.db.sql(
    "alter table public.nest_ai_commands add constraint reject_reminder check(tool_name <> 'saveChoreReminder')",
  );
  assert.throws(() => f.db.sql(as(f.reminderCommand(f.input))), /reject_reminder/);
  assert.equal(f.db.sql("select count(*) from public.nest_chore_reminders"), "0");
  assert.equal(f.db.sql("select count(*) from private.nest_chore_reminder_operations"), "0");
});
...
test("saved reminder transcript uses committed receipt instead of fabricated delivery claims", (t) => {
  ...
  const parts = transcript.at(-1).parts.filter((part) => part.type === "tool-saveChoreReminder");
  assert.equal(parts.length, 1);
  assert.deepEqual(parts[0].output, result);
  assert.equal(JSON.stringify(parts).includes('"delivered":true'), false);
});

test("private chore reminder history cannot cross owners and rejects revoked recipients", (t) => {
  const f = setup(t),
  ...
  f.db.sql(
    `update public.routine_occurrences set due_date=due_date+1 where id='${f.input.occurrenceId}'`,
  );
  ...
  const context = JSON.parse(
    f.db.sql(as(`select public.nest_read_chore_reminder('${id(10)}','${f.input.occurrenceId}')`)),
  );
  const changed = {
    ...f.input,

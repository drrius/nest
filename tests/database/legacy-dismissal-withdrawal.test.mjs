...
function fixture(t) {
  ...
  f.db.file("supabase/migrations/20260922021746_native_legacy_dismissal_withdrawal.sql");
  const input = f.input();
...
test("withdrawal and concurrent approval serialize to one truthful terminal outcome, fencing future retries", async (t) => {
  ...
  const recorded = f.db.sql("select count(*) from private.nest_legacy_draft_operations");
...
test("withdrawal revokes expired and separately approved consent without revealing another member's proposal", (t) => {
  ...
  f.db.sql(
    as(
      1,
      `select public.nest_decide_action('${f.approvalId}','${id(700)}','recurring.dismiss-legacy-draft',1,${json(f.input)},true)`,
    ),
  );

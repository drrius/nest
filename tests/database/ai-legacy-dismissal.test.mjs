...
import { fixture, as, id, json } from "./ai-legacy-dismissal-fixture.mjs";
import { matchesLegacyDismissalProposal } from "../../apps/api/src/money/legacy-dismissal-proposal.ts";
const member = { userId: id(1), householdId: id(10) };
test("concurrent AI proposals bind fresh draft state once; only native confirmation dismisses it", async (t) => {
  const f = fixture(t),
    turn = f.start();
  ...
  assert.equal(matchesLegacyDismissalProposal(f.input, value, member), true);
  assert.equal(matchesLegacyDismissalProposal({ draftId: id(901) }, value, member), false);
  assert.equal(matchesLegacyDismissalProposal(f.input, value, { ...member, userId: id(2) }), false);
  assert.equal(
    matchesLegacyDismissalProposal(f.input, value, { ...member, householdId: id(11) }),
    false,
  );
  ...
  const result = JSON.parse(
    f.db.sql(
      as(
        `select public.nest_decide_legacy_dismissal('${id(10)}','${approval.operationId}',${json(approval.input)},'${approval.id}',true)`,
      ),
    ),
  );
...
test("journal failure rolls back proposal and transcript reconciliation replaces model-fabricated output", (t) => {
  ...
  assert.throws(() => f.execute(turn, { draftId: id(901) }), /command changed/);
  const response = {
    id: f.db.sql(`select assistant_id from public.nest_ai_turns where operation_id='${turn.turn}'`),
    role: "assistant",
    parts: [
      {
        type: "tool-proposeLegacyDismissal",
        toolCallId: "draft",
        state: "output-available",
        input: f.input,
        output: { ok: true, value: { dismissed: true } },
      },
    ],
  };
  f.db.sql(
...
test("foreign, nonpending and model-injected approval fields cannot create dismissal proposals", (t) => {
  const f = fixture(t),
    turn = f.start();
  let call = 0;
  for (const patch of [
  ...
  for (const role of ["anon", "authenticated", "service_role"])
    assert.throws(
      () =>
        f.db.sql(
          `set role ${role}; select private.nest_propose_legacy_dismissal('${id(10)}','${id(701)}',${json(f.input)})`,
        ),
      /permission denied/,
    );
});
